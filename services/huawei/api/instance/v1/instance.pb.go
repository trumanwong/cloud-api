// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.28.0
// 	protoc        v3.20.0
// source: api/instance/v1/instance.proto

package v1

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type CreateInstancesRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	// 区域id
	RegionId string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	// project_id
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	// 镜像id
	ImageId string `protobuf:"bytes,5,opt,name=image_id,json=imageId,proto3" json:"image_id,omitempty"`
	// 实例名称只能由中文字符、英文字母、数字及“_”、“-”、“.”组成,且长度为[1-64]个字符。
	// 创建的云服务器器数量(count字段对应的值)大于1时,为区分不同云服务器,创建过程中系统会自动在名称后加“-0000”的类似标记。故此时名称的长度为[1-59]个字符。
	Name string `protobuf:"bytes,6,opt,name=name,proto3" json:"name,omitempty"`
	// 实例规格
	InstanceType string `protobuf:"bytes,7,opt,name=instance_type,json=instanceType,proto3" json:"instance_type,omitempty"`
	// 待创建云服务器所属虚拟私有云(简称VPC),需要指定已创建VPC的ID,UUID格式。
	VpcId string `protobuf:"bytes,8,opt,name=vpc_id,json=vpcId,proto3" json:"vpc_id,omitempty"`
	// 待创建云服务器的网卡信息。  约束：  - 网卡对应的子网（subnet）必须属于vpcid对应的VPC。 - 当前单个云服务器支持最多挂载12张网卡。 - 不同的规格对网卡上限有一定的区别，参考[规格清单](https://support.huaweicloud.com/productdesc-ecs/zh-cn_topic_0159822360.html)。
	Nics       []*CreateInstancesRequest_Nic      `protobuf:"bytes,9,rep,name=nics,proto3" json:"nics,omitempty"`
	RootVolume *CreateInstancesRequest_RootVolume `protobuf:"bytes,10,opt,name=root_volume,json=rootVolume,proto3" json:"root_volume,omitempty"`
	// 是否为instance_name自动添加有序后缀（默认false）
	UniqueSuffix bool `protobuf:"varint,11,opt,name=unique_suffix,json=uniqueSuffix,proto3" json:"unique_suffix,omitempty"`
	// 创建数量(1-100，默认1)
	// 不传该字段时默认取值为1。 租户的配额足够时,最大值为500。
	Amount int32 `protobuf:"varint,12,opt,name=amount,proto3" json:"amount,omitempty"`
	// 实例密码
	Password string `protobuf:"bytes,13,opt,name=password,proto3" json:"password,omitempty"`
	// 是否只预检此次请求
	// true：发送检查请求，不会创建实例。检查项包括是否填写了必需参数、请求格式等。
	// 如果检查不通过，则返回对应错误。
	// 如果检查通过，则返回202状态码。
	// false：发送正常请求，通过检查后并且执行创建云服务器请求。
	DryRun bool `protobuf:"varint,14,opt,name=dry_run,json=dryRun,proto3" json:"dry_run,omitempty"`
}

func (x *CreateInstancesRequest) Reset() {
	*x = CreateInstancesRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateInstancesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateInstancesRequest) ProtoMessage() {}

func (x *CreateInstancesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateInstancesRequest.ProtoReflect.Descriptor instead.
func (*CreateInstancesRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{0}
}

func (x *CreateInstancesRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *CreateInstancesRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *CreateInstancesRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *CreateInstancesRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *CreateInstancesRequest) GetImageId() string {
	if x != nil {
		return x.ImageId
	}
	return ""
}

func (x *CreateInstancesRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *CreateInstancesRequest) GetInstanceType() string {
	if x != nil {
		return x.InstanceType
	}
	return ""
}

func (x *CreateInstancesRequest) GetVpcId() string {
	if x != nil {
		return x.VpcId
	}
	return ""
}

func (x *CreateInstancesRequest) GetNics() []*CreateInstancesRequest_Nic {
	if x != nil {
		return x.Nics
	}
	return nil
}

func (x *CreateInstancesRequest) GetRootVolume() *CreateInstancesRequest_RootVolume {
	if x != nil {
		return x.RootVolume
	}
	return nil
}

func (x *CreateInstancesRequest) GetUniqueSuffix() bool {
	if x != nil {
		return x.UniqueSuffix
	}
	return false
}

func (x *CreateInstancesRequest) GetAmount() int32 {
	if x != nil {
		return x.Amount
	}
	return 0
}

func (x *CreateInstancesRequest) GetPassword() string {
	if x != nil {
		return x.Password
	}
	return ""
}

func (x *CreateInstancesRequest) GetDryRun() bool {
	if x != nil {
		return x.DryRun
	}
	return false
}

type CreateInstancesResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 提交任务成功后返回的任务ID，用户可以使用该ID对任务执行情况进行查询。
	JobId string `protobuf:"bytes,1,opt,name=job_id,json=jobId,proto3" json:"job_id,omitempty"`
	// 订单号，创建包年包月的弹性云服务器时返回该参数。
	OrderId string `protobuf:"bytes,2,opt,name=order_id,json=orderId,proto3" json:"order_id,omitempty"`
	// 云服务器ID列表。  通过云服务器ID查询云服务器详情 ，若返回404 可能云服务器还在创建或者已经创建失败。
	ServerIds []string `protobuf:"bytes,3,rep,name=server_ids,json=serverIds,proto3" json:"server_ids,omitempty"`
}

func (x *CreateInstancesResponse) Reset() {
	*x = CreateInstancesResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[1]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateInstancesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateInstancesResponse) ProtoMessage() {}

func (x *CreateInstancesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[1]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateInstancesResponse.ProtoReflect.Descriptor instead.
func (*CreateInstancesResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{1}
}

func (x *CreateInstancesResponse) GetJobId() string {
	if x != nil {
		return x.JobId
	}
	return ""
}

func (x *CreateInstancesResponse) GetOrderId() string {
	if x != nil {
		return x.OrderId
	}
	return ""
}

func (x *CreateInstancesResponse) GetServerIds() []string {
	if x != nil {
		return x.ServerIds
	}
	return nil
}

type ListInstancesRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	// 区域id
	RegionId string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	// project_id
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	// 查询返回云服务器当前页面的大小。每页最多返回1000台云服务器的信息。
	Limit int32 `protobuf:"varint,5,opt,name=limit,proto3" json:"limit,omitempty"`
	// 页码。 当前页面数，默认为1。  取值大于等于0，取值为0时返回第1页。
	Offset int32 `protobuf:"varint,6,opt,name=offset,proto3" json:"offset,omitempty"`
	// 云服务器名称,匹配规则为模糊匹配。
	Name string `protobuf:"bytes,7,opt,name=name,proto3" json:"name,omitempty"`
}

func (x *ListInstancesRequest) Reset() {
	*x = ListInstancesRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[2]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstancesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstancesRequest) ProtoMessage() {}

func (x *ListInstancesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[2]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstancesRequest.ProtoReflect.Descriptor instead.
func (*ListInstancesRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{2}
}

func (x *ListInstancesRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *ListInstancesRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *ListInstancesRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *ListInstancesRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *ListInstancesRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *ListInstancesRequest) GetOffset() int32 {
	if x != nil {
		return x.Offset
	}
	return 0
}

func (x *ListInstancesRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type ListInstancesResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 弹性云服务器详情列表,具体参照-查询云服务器详情接口。查询级别不同,返回的详情不同。
	Instances []*ListInstancesResponse_Instance `protobuf:"bytes,1,rep,name=instances,proto3" json:"instances,omitempty"`
	// 弹性云服务器的列表总数。
	Count int32 `protobuf:"varint,2,opt,name=count,proto3" json:"count,omitempty"`
}

func (x *ListInstancesResponse) Reset() {
	*x = ListInstancesResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[3]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstancesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstancesResponse) ProtoMessage() {}

func (x *ListInstancesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[3]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstancesResponse.ProtoReflect.Descriptor instead.
func (*ListInstancesResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{3}
}

func (x *ListInstancesResponse) GetInstances() []*ListInstancesResponse_Instance {
	if x != nil {
		return x.Instances
	}
	return nil
}

func (x *ListInstancesResponse) GetCount() int32 {
	if x != nil {
		return x.Count
	}
	return 0
}

type ListRegionsRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 查询地域类别，查询ims地域：image，查询ecs地域：ecs
	RegionType string `protobuf:"bytes,1,opt,name=region_type,json=regionType,proto3" json:"region_type,omitempty"`
}

func (x *ListRegionsRequest) Reset() {
	*x = ListRegionsRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[4]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListRegionsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRegionsRequest) ProtoMessage() {}

func (x *ListRegionsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[4]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRegionsRequest.ProtoReflect.Descriptor instead.
func (*ListRegionsRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{4}
}

func (x *ListRegionsRequest) GetRegionType() string {
	if x != nil {
		return x.RegionType
	}
	return ""
}

type ListRegionsResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Regions []*ListRegionsResponse_Region `protobuf:"bytes,1,rep,name=regions,proto3" json:"regions,omitempty"`
}

func (x *ListRegionsResponse) Reset() {
	*x = ListRegionsResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[5]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListRegionsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRegionsResponse) ProtoMessage() {}

func (x *ListRegionsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[5]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRegionsResponse.ProtoReflect.Descriptor instead.
func (*ListRegionsResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{5}
}

func (x *ListRegionsResponse) GetRegions() []*ListRegionsResponse_Region {
	if x != nil {
		return x.Regions
	}
	return nil
}

type ListImagesRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	RegionId  string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	// 用于分页,表示查询几条镜像记录,取值为整数,默认返回25条镜像记录
	Limit int32 `protobuf:"varint,4,opt,name=limit,proto3" json:"limit,omitempty"`
	// 用于分页,表示从哪个镜像开始查询,取值为镜像ID。
	Marker string `protobuf:"bytes,5,opt,name=marker,proto3" json:"marker,omitempty"`
}

func (x *ListImagesRequest) Reset() {
	*x = ListImagesRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[6]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListImagesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListImagesRequest) ProtoMessage() {}

func (x *ListImagesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[6]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListImagesRequest.ProtoReflect.Descriptor instead.
func (*ListImagesRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{6}
}

func (x *ListImagesRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *ListImagesRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *ListImagesRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *ListImagesRequest) GetLimit() int32 {
	if x != nil {
		return x.Limit
	}
	return 0
}

func (x *ListImagesRequest) GetMarker() string {
	if x != nil {
		return x.Marker
	}
	return ""
}

type ListImagesResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Images []*ListImagesResponse_Image `protobuf:"bytes,1,rep,name=images,proto3" json:"images,omitempty"`
	// 用于分页,表示从哪个镜像开始查询,取值为镜像ID。
	Marker string `protobuf:"bytes,2,opt,name=marker,proto3" json:"marker,omitempty"`
}

func (x *ListImagesResponse) Reset() {
	*x = ListImagesResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[7]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListImagesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListImagesResponse) ProtoMessage() {}

func (x *ListImagesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[7]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListImagesResponse.ProtoReflect.Descriptor instead.
func (*ListImagesResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{7}
}

func (x *ListImagesResponse) GetImages() []*ListImagesResponse_Image {
	if x != nil {
		return x.Images
	}
	return nil
}

func (x *ListImagesResponse) GetMarker() string {
	if x != nil {
		return x.Marker
	}
	return ""
}

type ListInstanceTypesRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	RegionId  string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
}

func (x *ListInstanceTypesRequest) Reset() {
	*x = ListInstanceTypesRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[8]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstanceTypesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstanceTypesRequest) ProtoMessage() {}

func (x *ListInstanceTypesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[8]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstanceTypesRequest.ProtoReflect.Descriptor instead.
func (*ListInstanceTypesRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{8}
}

func (x *ListInstanceTypesRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *ListInstanceTypesRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *ListInstanceTypesRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *ListInstanceTypesRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

type ListInstanceTypesResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	InstanceTypes []*ListInstanceTypesResponse_InstanceType `protobuf:"bytes,1,rep,name=instance_types,json=instanceTypes,proto3" json:"instance_types,omitempty"`
}

func (x *ListInstanceTypesResponse) Reset() {
	*x = ListInstanceTypesResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[9]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstanceTypesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstanceTypesResponse) ProtoMessage() {}

func (x *ListInstanceTypesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[9]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstanceTypesResponse.ProtoReflect.Descriptor instead.
func (*ListInstanceTypesResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{9}
}

func (x *ListInstanceTypesResponse) GetInstanceTypes() []*ListInstanceTypesResponse_InstanceType {
	if x != nil {
		return x.InstanceTypes
	}
	return nil
}

type StartInstancesRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	RegionId  string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	// 云服务器id列表
	Servers []*ServerId `protobuf:"bytes,5,rep,name=servers,proto3" json:"servers,omitempty"`
}

func (x *StartInstancesRequest) Reset() {
	*x = StartInstancesRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[10]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *StartInstancesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StartInstancesRequest) ProtoMessage() {}

func (x *StartInstancesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[10]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StartInstancesRequest.ProtoReflect.Descriptor instead.
func (*StartInstancesRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{10}
}

func (x *StartInstancesRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *StartInstancesRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *StartInstancesRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *StartInstancesRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *StartInstancesRequest) GetServers() []*ServerId {
	if x != nil {
		return x.Servers
	}
	return nil
}

type StartInstancesResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	JobId string `protobuf:"bytes,1,opt,name=job_id,json=jobId,proto3" json:"job_id,omitempty"`
}

func (x *StartInstancesResponse) Reset() {
	*x = StartInstancesResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[11]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *StartInstancesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StartInstancesResponse) ProtoMessage() {}

func (x *StartInstancesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[11]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StartInstancesResponse.ProtoReflect.Descriptor instead.
func (*StartInstancesResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{11}
}

func (x *StartInstancesResponse) GetJobId() string {
	if x != nil {
		return x.JobId
	}
	return ""
}

type StopInstancesRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	RegionId  string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	// 云服务器id列表
	Servers []*ServerId `protobuf:"bytes,5,rep,name=servers,proto3" json:"servers,omitempty"`
	// 关机类型，默认为SOFT：  - SOFT：普通关机（默认）。 - HARD：强制关机。
	StopType string `protobuf:"bytes,6,opt,name=stop_type,json=stopType,proto3" json:"stop_type,omitempty"`
}

func (x *StopInstancesRequest) Reset() {
	*x = StopInstancesRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[12]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *StopInstancesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StopInstancesRequest) ProtoMessage() {}

func (x *StopInstancesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[12]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StopInstancesRequest.ProtoReflect.Descriptor instead.
func (*StopInstancesRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{12}
}

func (x *StopInstancesRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *StopInstancesRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *StopInstancesRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *StopInstancesRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *StopInstancesRequest) GetServers() []*ServerId {
	if x != nil {
		return x.Servers
	}
	return nil
}

func (x *StopInstancesRequest) GetStopType() string {
	if x != nil {
		return x.StopType
	}
	return ""
}

type StopInstancesResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	JobId string `protobuf:"bytes,1,opt,name=job_id,json=jobId,proto3" json:"job_id,omitempty"`
}

func (x *StopInstancesResponse) Reset() {
	*x = StopInstancesResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[13]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *StopInstancesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*StopInstancesResponse) ProtoMessage() {}

func (x *StopInstancesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[13]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use StopInstancesResponse.ProtoReflect.Descriptor instead.
func (*StopInstancesResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{13}
}

func (x *StopInstancesResponse) GetJobId() string {
	if x != nil {
		return x.JobId
	}
	return ""
}

type RebootInstancesRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	RegionId  string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	// 云服务器id列表
	Servers []*ServerId `protobuf:"bytes,5,rep,name=servers,proto3" json:"servers,omitempty"`
	// 重启类型，默认为SOFT：  - SOFT：普通关机（默认）。 - HARD：强制关机。
	RestartType string `protobuf:"bytes,6,opt,name=restart_type,json=restartType,proto3" json:"restart_type,omitempty"`
}

func (x *RebootInstancesRequest) Reset() {
	*x = RebootInstancesRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[14]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *RebootInstancesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RebootInstancesRequest) ProtoMessage() {}

func (x *RebootInstancesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[14]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RebootInstancesRequest.ProtoReflect.Descriptor instead.
func (*RebootInstancesRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{14}
}

func (x *RebootInstancesRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *RebootInstancesRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *RebootInstancesRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *RebootInstancesRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *RebootInstancesRequest) GetServers() []*ServerId {
	if x != nil {
		return x.Servers
	}
	return nil
}

func (x *RebootInstancesRequest) GetRestartType() string {
	if x != nil {
		return x.RestartType
	}
	return ""
}

type RebootInstancesResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	JobId string `protobuf:"bytes,1,opt,name=job_id,json=jobId,proto3" json:"job_id,omitempty"`
}

func (x *RebootInstancesResponse) Reset() {
	*x = RebootInstancesResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[15]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *RebootInstancesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*RebootInstancesResponse) ProtoMessage() {}

func (x *RebootInstancesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[15]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use RebootInstancesResponse.ProtoReflect.Descriptor instead.
func (*RebootInstancesResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{15}
}

func (x *RebootInstancesResponse) GetJobId() string {
	if x != nil {
		return x.JobId
	}
	return ""
}

type ServerId struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 云服务器ID。
	Id string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
}

func (x *ServerId) Reset() {
	*x = ServerId{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[16]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ServerId) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ServerId) ProtoMessage() {}

func (x *ServerId) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[16]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ServerId.ProtoReflect.Descriptor instead.
func (*ServerId) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{16}
}

func (x *ServerId) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

type DeleteInstancesRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	RegionId  string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	// 配置删除云服务器是否删除云服务器绑定的弹性IP。如果选择不删除，则系统仅做解绑定操作，保留弹性IP资源。 取值为true或false，默认为false。   - true：删除云服务器时会同时删除绑定在云服务器上的弹性IP。  - false：删除云服务器时，仅解绑定在云服务器上的弹性IP，不删除弹性IP
	DeletePublicIp bool `protobuf:"varint,5,opt,name=delete_public_ip,json=deletePublicIp,proto3" json:"delete_public_ip,omitempty"`
	// 配置删除云服务器是否删除云服务器对应的数据盘，如果选择不删除，则系统仅做解绑定操作，保留云磁盘资源。 取值为false或true，默认为false。  - true：删除云服务器时会同时删除挂载在云服务器上的数据盘。 - false：删除云服务器时，仅卸载云服务器上挂载的数据盘，不删除该数据盘。
	DeleteVolume bool `protobuf:"varint,6,opt,name=delete_volume,json=deleteVolume,proto3" json:"delete_volume,omitempty"`
	// 所需要删除的云服务器列表。
	Servers []*ServerId `protobuf:"bytes,7,rep,name=servers,proto3" json:"servers,omitempty"`
}

func (x *DeleteInstancesRequest) Reset() {
	*x = DeleteInstancesRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[17]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *DeleteInstancesRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteInstancesRequest) ProtoMessage() {}

func (x *DeleteInstancesRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[17]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteInstancesRequest.ProtoReflect.Descriptor instead.
func (*DeleteInstancesRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{17}
}

func (x *DeleteInstancesRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *DeleteInstancesRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *DeleteInstancesRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *DeleteInstancesRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *DeleteInstancesRequest) GetDeletePublicIp() bool {
	if x != nil {
		return x.DeletePublicIp
	}
	return false
}

func (x *DeleteInstancesRequest) GetDeleteVolume() bool {
	if x != nil {
		return x.DeleteVolume
	}
	return false
}

func (x *DeleteInstancesRequest) GetServers() []*ServerId {
	if x != nil {
		return x.Servers
	}
	return nil
}

type DeleteInstancesResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	JobId string `protobuf:"bytes,1,opt,name=job_id,json=jobId,proto3" json:"job_id,omitempty"`
}

func (x *DeleteInstancesResponse) Reset() {
	*x = DeleteInstancesResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[18]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *DeleteInstancesResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteInstancesResponse) ProtoMessage() {}

func (x *DeleteInstancesResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[18]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteInstancesResponse.ProtoReflect.Descriptor instead.
func (*DeleteInstancesResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{18}
}

func (x *DeleteInstancesResponse) GetJobId() string {
	if x != nil {
		return x.JobId
	}
	return ""
}

type Vpc struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 功能说明：虚拟私有云ID 取值范围：带\"-\"的UUID
	Id string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	// 功能说明：虚拟私有云名称 取值范围：0-64个字符，支持数字、字母、中文、_(下划线)、-（中划线）、.（点） 约束：如果名称不为空，则同一个租户下的名称不能重复
	Name string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// 功能说明：虚拟私有云下可用子网的范围 取值范围： - 10.0.0.0/8~10.255.255.240/28 - 172.16.0.0/12 ~ 172.31.255.240/28 - 192.168.0.0/16 ~ 192.168.255.240/28 不指定cidr时，默认值为空 约束：必须是ipv4 cidr格式，例如:192.168.0.0/16
	Cidr string `protobuf:"bytes,3,opt,name=cidr,proto3" json:"cidr,omitempty"`
	// 功能说明：虚拟私有云的描述 取值范围：0-255个字符，不能包含“<”和“>”
	Description string `protobuf:"bytes,4,opt,name=description,proto3" json:"description,omitempty"`
	// 功能说明：路由信息列表，详情参见route对象
	Routes []*VpcRoute `protobuf:"bytes,5,rep,name=routes,proto3" json:"routes,omitempty"`
	// 功能说明：虚拟私有云的状态 取值范围： - CREATING：创建中 - OK：创建成功
	Status string `protobuf:"bytes,6,opt,name=status,proto3" json:"status,omitempty"`
	// 功能说明：企业项目ID。 取值范围：最大长度36字节，带“-”连字符的UUID格式，或者是字符串“0”。“0”表示默认企业项目。
	EnterpriseProjectId string `protobuf:"bytes,7,opt,name=enterprise_project_id,json=enterpriseProjectId,proto3" json:"enterprise_project_id,omitempty"`
}

func (x *Vpc) Reset() {
	*x = Vpc{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[19]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Vpc) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Vpc) ProtoMessage() {}

func (x *Vpc) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[19]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Vpc.ProtoReflect.Descriptor instead.
func (*Vpc) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{19}
}

func (x *Vpc) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *Vpc) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Vpc) GetCidr() string {
	if x != nil {
		return x.Cidr
	}
	return ""
}

func (x *Vpc) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *Vpc) GetRoutes() []*VpcRoute {
	if x != nil {
		return x.Routes
	}
	return nil
}

func (x *Vpc) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

func (x *Vpc) GetEnterpriseProjectId() string {
	if x != nil {
		return x.EnterpriseProjectId
	}
	return ""
}

type VpcRoute struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 功能说明：路由目的地 取值范围：IP地址段 约束：仅支持配置默认路由，且其取值只能是0.0.0.0/0
	Destination string `protobuf:"bytes,1,opt,name=destination,proto3" json:"destination,omitempty"`
	// 功能说明：路由下一跳IP地址 取值范围：ipv4地址格式 约束：nexthop仅支持所关联的子网范围内IP地址
	NextHop string `protobuf:"bytes,2,opt,name=next_hop,json=nextHop,proto3" json:"next_hop,omitempty"`
}

func (x *VpcRoute) Reset() {
	*x = VpcRoute{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[20]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *VpcRoute) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*VpcRoute) ProtoMessage() {}

func (x *VpcRoute) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[20]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use VpcRoute.ProtoReflect.Descriptor instead.
func (*VpcRoute) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{20}
}

func (x *VpcRoute) GetDestination() string {
	if x != nil {
		return x.Destination
	}
	return ""
}

func (x *VpcRoute) GetNextHop() string {
	if x != nil {
		return x.NextHop
	}
	return ""
}

type CreateVpcRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	RegionId  string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	// 功能说明:虚拟私有云下可用子网的范围
	//  取值范围:
	//    10.0.0.0/8 ~ 10.255.255.240/28
	//    172.16.0.0/12 ~ 172.31.255.240/28
	//    192.168.0.0/16 ~ 192.168.255.240/28 约束:必须是ipv4 cidr格式,例如:192.168.0.0/16
	Cidr string `protobuf:"bytes,5,opt,name=cidr,proto3" json:"cidr,omitempty"`
	// 虚拟私有云名称
	//  取值范围:0-64个字符,支持数字、字母、中文、_(下划线)、-(中划线)、.(点)
	//  约束:如果名称不为空,则同一个租户下的名称不能重复
	Name string `protobuf:"bytes,6,opt,name=name,proto3" json:"name,omitempty"`
	// 虚拟私有云的描述
	//  取值范围:0-255个字符,不能包含“<”和“>”。
	Description string `protobuf:"bytes,7,opt,name=description,proto3" json:"description,omitempty"`
	// 企业项目ID。创建虚拟私有云时,给虚拟私有云绑定企业项目ID。
	//  取值范围:最大长度36字节,带“-”连字符的UUID格式,或者是字符串“0”。“0”表示默认企业项目。
	//  默认值:"0"
	EnterpriseProjectId string `protobuf:"bytes,8,opt,name=enterprise_project_id,json=enterpriseProjectId,proto3" json:"enterprise_project_id,omitempty"`
}

func (x *CreateVpcRequest) Reset() {
	*x = CreateVpcRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[21]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateVpcRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateVpcRequest) ProtoMessage() {}

func (x *CreateVpcRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[21]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateVpcRequest.ProtoReflect.Descriptor instead.
func (*CreateVpcRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{21}
}

func (x *CreateVpcRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *CreateVpcRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *CreateVpcRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *CreateVpcRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *CreateVpcRequest) GetCidr() string {
	if x != nil {
		return x.Cidr
	}
	return ""
}

func (x *CreateVpcRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *CreateVpcRequest) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *CreateVpcRequest) GetEnterpriseProjectId() string {
	if x != nil {
		return x.EnterpriseProjectId
	}
	return ""
}

type CreateVpcResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Vpc *Vpc `protobuf:"bytes,1,opt,name=vpc,proto3" json:"vpc,omitempty"`
}

func (x *CreateVpcResponse) Reset() {
	*x = CreateVpcResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[22]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateVpcResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateVpcResponse) ProtoMessage() {}

func (x *CreateVpcResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[22]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateVpcResponse.ProtoReflect.Descriptor instead.
func (*CreateVpcResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{22}
}

func (x *CreateVpcResponse) GetVpc() *Vpc {
	if x != nil {
		return x.Vpc
	}
	return nil
}

type ListVpcsRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	RegionId  string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	// 功能说明：虚拟私有云ID。可以使用该字段过滤某个ID的虚拟私有云。
	Id string `protobuf:"bytes,5,opt,name=id,proto3" json:"id,omitempty"`
	// 功能说明：企业项目ID。可以使用该字段过滤某个企业项目下的虚拟私有云。若未传值则默认返回所有企业项目绑定的虚拟私有云。  取值范围：最大长度36字节，带“-”连字符的UUID格式，或者是字符串“0”。“0”表示默认企业项目。若需要查询当前用户所有企业项目绑定的虚拟私有云，请传参all_granted_eps。
	EnterpriseProjectId string `protobuf:"bytes,6,opt,name=enterprise_project_id,json=enterpriseProjectId,proto3" json:"enterprise_project_id,omitempty"`
}

func (x *ListVpcsRequest) Reset() {
	*x = ListVpcsRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[23]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListVpcsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListVpcsRequest) ProtoMessage() {}

func (x *ListVpcsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[23]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListVpcsRequest.ProtoReflect.Descriptor instead.
func (*ListVpcsRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{23}
}

func (x *ListVpcsRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *ListVpcsRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *ListVpcsRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *ListVpcsRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *ListVpcsRequest) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *ListVpcsRequest) GetEnterpriseProjectId() string {
	if x != nil {
		return x.EnterpriseProjectId
	}
	return ""
}

type ListVpcsResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Vpcs []*Vpc `protobuf:"bytes,1,rep,name=vpcs,proto3" json:"vpcs,omitempty"`
}

func (x *ListVpcsResponse) Reset() {
	*x = ListVpcsResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[24]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListVpcsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListVpcsResponse) ProtoMessage() {}

func (x *ListVpcsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[24]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListVpcsResponse.ProtoReflect.Descriptor instead.
func (*ListVpcsResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{24}
}

func (x *ListVpcsResponse) GetVpcs() []*Vpc {
	if x != nil {
		return x.Vpcs
	}
	return nil
}

type UpdateVpcRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	RegionId  string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	// 虚拟私有云ID
	VpcId string `protobuf:"bytes,5,opt,name=vpc_id,json=vpcId,proto3" json:"vpc_id,omitempty"`
	// 功能说明:虚拟私有云下可用子网的范围
	//  取值范围:
	//    10.0.0.0/8 ~ 10.255.255.240/28
	//    172.16.0.0/12 ~ 172.31.255.240/28
	//    192.168.0.0/16 ~ 192.168.255.240/28 约束:必须是ipv4 cidr格式,例如:192.168.0.0/16
	Cidr string `protobuf:"bytes,6,opt,name=cidr,proto3" json:"cidr,omitempty"`
	// 虚拟私有云名称
	//  取值范围:0-64个字符,支持数字、字母、中文、_(下划线)、-(中划线)、.(点)
	//  约束:如果名称不为空,则同一个租户下的名称不能重复
	Name string `protobuf:"bytes,7,opt,name=name,proto3" json:"name,omitempty"`
	// 虚拟私有云的描述
	//  取值范围:0-255个字符,不能包含“<”和“>”。
	Description string `protobuf:"bytes,8,opt,name=description,proto3" json:"description,omitempty"`
	// 路由信息列表
	Routes []*VpcRoute `protobuf:"bytes,9,rep,name=routes,proto3" json:"routes,omitempty"`
}

func (x *UpdateVpcRequest) Reset() {
	*x = UpdateVpcRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[25]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateVpcRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateVpcRequest) ProtoMessage() {}

func (x *UpdateVpcRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[25]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateVpcRequest.ProtoReflect.Descriptor instead.
func (*UpdateVpcRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{25}
}

func (x *UpdateVpcRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *UpdateVpcRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *UpdateVpcRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *UpdateVpcRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *UpdateVpcRequest) GetVpcId() string {
	if x != nil {
		return x.VpcId
	}
	return ""
}

func (x *UpdateVpcRequest) GetCidr() string {
	if x != nil {
		return x.Cidr
	}
	return ""
}

func (x *UpdateVpcRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *UpdateVpcRequest) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *UpdateVpcRequest) GetRoutes() []*VpcRoute {
	if x != nil {
		return x.Routes
	}
	return nil
}

type UpdateVpcResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Vpc *Vpc `protobuf:"bytes,1,opt,name=vpc,proto3" json:"vpc,omitempty"`
}

func (x *UpdateVpcResponse) Reset() {
	*x = UpdateVpcResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[26]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateVpcResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateVpcResponse) ProtoMessage() {}

func (x *UpdateVpcResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[26]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateVpcResponse.ProtoReflect.Descriptor instead.
func (*UpdateVpcResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{26}
}

func (x *UpdateVpcResponse) GetVpc() *Vpc {
	if x != nil {
		return x.Vpc
	}
	return nil
}

type DeleteVpcRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	RegionId  string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	VpcId     string `protobuf:"bytes,5,opt,name=vpc_id,json=vpcId,proto3" json:"vpc_id,omitempty"`
}

func (x *DeleteVpcRequest) Reset() {
	*x = DeleteVpcRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[27]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *DeleteVpcRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteVpcRequest) ProtoMessage() {}

func (x *DeleteVpcRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[27]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteVpcRequest.ProtoReflect.Descriptor instead.
func (*DeleteVpcRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{27}
}

func (x *DeleteVpcRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *DeleteVpcRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *DeleteVpcRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *DeleteVpcRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *DeleteVpcRequest) GetVpcId() string {
	if x != nil {
		return x.VpcId
	}
	return ""
}

type DeleteVpcResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
}

func (x *DeleteVpcResponse) Reset() {
	*x = DeleteVpcResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[28]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *DeleteVpcResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteVpcResponse) ProtoMessage() {}

func (x *DeleteVpcResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[28]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteVpcResponse.ProtoReflect.Descriptor instead.
func (*DeleteVpcResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{28}
}

type ShowVpcRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	RegionId  string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	VpcId     string `protobuf:"bytes,5,opt,name=vpc_id,json=vpcId,proto3" json:"vpc_id,omitempty"`
}

func (x *ShowVpcRequest) Reset() {
	*x = ShowVpcRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[29]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ShowVpcRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ShowVpcRequest) ProtoMessage() {}

func (x *ShowVpcRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[29]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ShowVpcRequest.ProtoReflect.Descriptor instead.
func (*ShowVpcRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{29}
}

func (x *ShowVpcRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *ShowVpcRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *ShowVpcRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *ShowVpcRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *ShowVpcRequest) GetVpcId() string {
	if x != nil {
		return x.VpcId
	}
	return ""
}

type ShowVpcResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Vpc *Vpc `protobuf:"bytes,1,opt,name=vpc,proto3" json:"vpc,omitempty"`
}

func (x *ShowVpcResponse) Reset() {
	*x = ShowVpcResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[30]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ShowVpcResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ShowVpcResponse) ProtoMessage() {}

func (x *ShowVpcResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[30]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ShowVpcResponse.ProtoReflect.Descriptor instead.
func (*ShowVpcResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{30}
}

func (x *ShowVpcResponse) GetVpc() *Vpc {
	if x != nil {
		return x.Vpc
	}
	return nil
}

type Subnet struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 子网ID
	Id string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	// 功能说明：子网名称 取值范围：1-64个字符，支持数字、字母、中文、_(下划线)、-（中划线）、.（点）
	Name string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// 功能说明：子网描述 取值范围：0-255个字符，不能包含“<”和“>”。
	Description string `protobuf:"bytes,3,opt,name=description,proto3" json:"description,omitempty"`
	// 功能说明：子网的网段 取值范围：必须在vpc对应cidr范围内 约束：必须是cidr格式。掩码长度不能大于28
	Cidr string `protobuf:"bytes,4,opt,name=cidr,proto3" json:"cidr,omitempty"`
	// 功能说明：子网的网关 取值范围：子网网段中的IP地址 约束：必须是ip格式
	GatewayIp string `protobuf:"bytes,5,opt,name=gateway_ip,json=gatewayIp,proto3" json:"gateway_ip,omitempty"`
	// 功能说明：是否创建cidr_v6 取值范围：true（开启），false（关闭）
	Ipv6Enable bool `protobuf:"varint,6,opt,name=ipv6_enable,json=ipv6Enable,proto3" json:"ipv6_enable,omitempty"`
	// IPv6子网的网段，如果子网为IPv4子网，则不返回此参数
	CidrV6 string `protobuf:"bytes,7,opt,name=cidr_v6,json=cidrV6,proto3" json:"cidr_v6,omitempty"`
	// IPv6子网的网关，如果子网为IPv4子网，则不返回此参数
	GatewayIpV6 string `protobuf:"bytes,8,opt,name=gateway_ip_v6,json=gatewayIpV6,proto3" json:"gateway_ip_v6,omitempty"`
	// 子网是否开启dhcp功能
	DhcpEnable bool `protobuf:"varint,9,opt,name=dhcp_enable,json=dhcpEnable,proto3" json:"dhcp_enable,omitempty"`
	// 子网dns服务器地址1
	PrimaryDns string `protobuf:"bytes,10,opt,name=primary_dns,json=primaryDns,proto3" json:"primary_dns,omitempty"`
	// 子网dns服务器地址2
	SecondaryDns string `protobuf:"bytes,11,opt,name=secondary_dns,json=secondaryDns,proto3" json:"secondary_dns,omitempty"`
	// 子网dns服务器地址列表
	DnsList []string `protobuf:"bytes,12,rep,name=dns_list,json=dnsList,proto3" json:"dns_list,omitempty"`
	// 子网所在的可用区标识
	AvailabilityZone string `protobuf:"bytes,13,opt,name=availability_zone,json=availabilityZone,proto3" json:"availability_zone,omitempty"`
	// 子网所在VPC标识
	VpcId string `protobuf:"bytes,14,opt,name=vpc_id,json=vpcId,proto3" json:"vpc_id,omitempty"`
	// 功能说明：子网的状态 取值范围： - ACTIVE：表示子网已挂载到ROUTER上 - UNKNOWN：表示子网还未挂载到ROUTER上 - ERROR：表示子网状态故障
	Status string `protobuf:"bytes,15,opt,name=status,proto3" json:"status,omitempty"`
	// 对应网络（OpenStack Neutron接口）id
	NeutronNetworkId string `protobuf:"bytes,16,opt,name=neutron_network_id,json=neutronNetworkId,proto3" json:"neutron_network_id,omitempty"`
	// 对应子网（OpenStack Neutron接口）id
	NeutronSubnetId string `protobuf:"bytes,17,opt,name=neutron_subnet_id,json=neutronSubnetId,proto3" json:"neutron_subnet_id,omitempty"`
	// 对应IPv6子网（OpenStack Neutron接口）id，如果子网为IPv4子网，则不返回此参数
	NeutronSubnetIdV6 string           `protobuf:"bytes,18,opt,name=neutron_subnet_id_v6,json=neutronSubnetIdV6,proto3" json:"neutron_subnet_id_v6,omitempty"`
	ExtraDhcpOpts     []*ExtraDhcpOpts `protobuf:"bytes,19,rep,name=extra_dhcp_opts,json=extraDhcpOpts,proto3" json:"extra_dhcp_opts,omitempty"`
	// 功能说明：子网作用域 取值范围：center-表示作用域为中心；{azId}表示作用域为具体的AZ
	Scope string `protobuf:"bytes,20,opt,name=scope,proto3" json:"scope,omitempty"`
}

func (x *Subnet) Reset() {
	*x = Subnet{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[31]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *Subnet) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Subnet) ProtoMessage() {}

func (x *Subnet) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[31]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Subnet.ProtoReflect.Descriptor instead.
func (*Subnet) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{31}
}

func (x *Subnet) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *Subnet) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *Subnet) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *Subnet) GetCidr() string {
	if x != nil {
		return x.Cidr
	}
	return ""
}

func (x *Subnet) GetGatewayIp() string {
	if x != nil {
		return x.GatewayIp
	}
	return ""
}

func (x *Subnet) GetIpv6Enable() bool {
	if x != nil {
		return x.Ipv6Enable
	}
	return false
}

func (x *Subnet) GetCidrV6() string {
	if x != nil {
		return x.CidrV6
	}
	return ""
}

func (x *Subnet) GetGatewayIpV6() string {
	if x != nil {
		return x.GatewayIpV6
	}
	return ""
}

func (x *Subnet) GetDhcpEnable() bool {
	if x != nil {
		return x.DhcpEnable
	}
	return false
}

func (x *Subnet) GetPrimaryDns() string {
	if x != nil {
		return x.PrimaryDns
	}
	return ""
}

func (x *Subnet) GetSecondaryDns() string {
	if x != nil {
		return x.SecondaryDns
	}
	return ""
}

func (x *Subnet) GetDnsList() []string {
	if x != nil {
		return x.DnsList
	}
	return nil
}

func (x *Subnet) GetAvailabilityZone() string {
	if x != nil {
		return x.AvailabilityZone
	}
	return ""
}

func (x *Subnet) GetVpcId() string {
	if x != nil {
		return x.VpcId
	}
	return ""
}

func (x *Subnet) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

func (x *Subnet) GetNeutronNetworkId() string {
	if x != nil {
		return x.NeutronNetworkId
	}
	return ""
}

func (x *Subnet) GetNeutronSubnetId() string {
	if x != nil {
		return x.NeutronSubnetId
	}
	return ""
}

func (x *Subnet) GetNeutronSubnetIdV6() string {
	if x != nil {
		return x.NeutronSubnetIdV6
	}
	return ""
}

func (x *Subnet) GetExtraDhcpOpts() []*ExtraDhcpOpts {
	if x != nil {
		return x.ExtraDhcpOpts
	}
	return nil
}

func (x *Subnet) GetScope() string {
	if x != nil {
		return x.Scope
	}
	return ""
}

// 子网配置的NTP地址
type ExtraDhcpOpts struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 功能说明：子网配置的NTP地址名称 约束：目前只支持字段“ntp”
	OptName string `protobuf:"bytes,1,opt,name=opt_name,json=optName,proto3" json:"opt_name,omitempty"`
	// 功能说明：子网配置的NTP地址 约束：目前只支持IPv4地址，每个IP地址以逗号隔开，IP地址个数不能超过4个，不能存在相同地址。该字段为null表示取消该子网NTP的设置，不能为””(空字符串)。
	OptValue string `protobuf:"bytes,2,opt,name=opt_value,json=optValue,proto3" json:"opt_value,omitempty"`
}

func (x *ExtraDhcpOpts) Reset() {
	*x = ExtraDhcpOpts{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[32]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ExtraDhcpOpts) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ExtraDhcpOpts) ProtoMessage() {}

func (x *ExtraDhcpOpts) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[32]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ExtraDhcpOpts.ProtoReflect.Descriptor instead.
func (*ExtraDhcpOpts) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{32}
}

func (x *ExtraDhcpOpts) GetOptName() string {
	if x != nil {
		return x.OptName
	}
	return ""
}

func (x *ExtraDhcpOpts) GetOptValue() string {
	if x != nil {
		return x.OptValue
	}
	return ""
}

type CreateSubnetRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	RegionId  string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	// 功能说明：子网的网段 取值范围：必须在vpc对应cidr范围内 约束：必须是cidr格式。掩码长度不能大于28
	Cidr string `protobuf:"bytes,5,opt,name=cidr,proto3" json:"cidr,omitempty"`
	// 功能说明：子网名称 取值范围：1-64个字符，支持数字、字母、中文、_(下划线)、-（中划线）、.（点）
	Name string `protobuf:"bytes,6,opt,name=name,proto3" json:"name,omitempty"`
	// 子网所在VPC标识
	VpcId string `protobuf:"bytes,7,opt,name=vpc_id,json=vpcId,proto3" json:"vpc_id,omitempty"`
	// 功能说明：子网的网关 取值范围：子网网段中的IP地址 约束：必须是ip格式
	GatewayIp string `protobuf:"bytes,8,opt,name=gateway_ip,json=gatewayIp,proto3" json:"gateway_ip,omitempty"`
	// 功能说明：子网描述 取值范围：0-255个字符，不能包含“<”和“>”。
	Description string `protobuf:"bytes,9,opt,name=description,proto3" json:"description,omitempty"`
	// 功能说明：是否创建cidr_v6 取值范围：true（开启），false（关闭） 约束：不填时默认为false > 说明 该参数目前仅在“华北-北京四”区域开放，且申请IPv6公测后才可设置。
	Ipv6Enable bool `protobuf:"varint,10,opt,name=ipv6_enable,json=ipv6Enable,proto3" json:"ipv6_enable,omitempty"`
	// 功能说明：子网是否开启dhcp功能 取值范围：true（开启），false（关闭） 约束：不填时默认为true。当设置为false时，会导致新创建的ECS无法获取IP地址，cloudinit无法注入账号密码，请谨慎操作。
	DhcpEnable bool `protobuf:"varint,11,opt,name=dhcp_enable,json=dhcpEnable,proto3" json:"dhcp_enable,omitempty"`
	// 功能说明：子网dns服务器地址1 约束：ip格式，不支持IPv6地址 默认值：不填时为空 [内网DNS地址请参见](https://support.huaweicloud.com/dns_faq/dns_faq_002.html) [通过API获取请参见](https://support.huaweicloud.com/api-dns/dns_api_69001.html)
	PrimaryDns string `protobuf:"bytes,12,opt,name=primary_dns,json=primaryDns,proto3" json:"primary_dns,omitempty"`
	// 功能说明：子网dns服务器地址2 约束：ip格式，不支持IPv6地址 默认值：不填时为空 [内网DNS地址请参见](https://support.huaweicloud.com/dns_faq/dns_faq_002.html) [通过API获取请参见](https://support.huaweicloud.com/api-dns/dns_api_69001.html)
	SecondaryDns string `protobuf:"bytes,13,opt,name=secondary_dns,json=secondaryDns,proto3" json:"secondary_dns,omitempty"`
	// 功能说明：子网dns服务器地址的集合；如果想使用两个以上dns服务器，请使用该字段 约束：是子网dns服务器地址1跟子网dns服务器地址2的合集的父集，不支持IPv6地址。 默认值：不填时为空，无法使用云内网DNS功能 [内网DNS地址请参见](https://support.huaweicloud.com/dns_faq/dns_faq_002.html) [通过API获取请参见](https://support.huaweicloud.com/api-dns/dns_api_69001.html)
	DnsList []string `protobuf:"bytes,14,rep,name=dns_list,json=dnsList,proto3" json:"dns_list,omitempty"`
	// 功能说明：子网所在的可用分区标识 约束：系统存在的可用分区标识
	AvailabilityZone string `protobuf:"bytes,15,opt,name=availability_zone,json=availabilityZone,proto3" json:"availability_zone,omitempty"`
	// 子网配置的NTP地址
	ExtraDhcpOpts []*ExtraDhcpOpts `protobuf:"bytes,16,rep,name=extra_dhcp_opts,json=extraDhcpOpts,proto3" json:"extra_dhcp_opts,omitempty"`
}

func (x *CreateSubnetRequest) Reset() {
	*x = CreateSubnetRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[33]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateSubnetRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateSubnetRequest) ProtoMessage() {}

func (x *CreateSubnetRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[33]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateSubnetRequest.ProtoReflect.Descriptor instead.
func (*CreateSubnetRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{33}
}

func (x *CreateSubnetRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *CreateSubnetRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *CreateSubnetRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *CreateSubnetRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *CreateSubnetRequest) GetCidr() string {
	if x != nil {
		return x.Cidr
	}
	return ""
}

func (x *CreateSubnetRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *CreateSubnetRequest) GetVpcId() string {
	if x != nil {
		return x.VpcId
	}
	return ""
}

func (x *CreateSubnetRequest) GetGatewayIp() string {
	if x != nil {
		return x.GatewayIp
	}
	return ""
}

func (x *CreateSubnetRequest) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *CreateSubnetRequest) GetIpv6Enable() bool {
	if x != nil {
		return x.Ipv6Enable
	}
	return false
}

func (x *CreateSubnetRequest) GetDhcpEnable() bool {
	if x != nil {
		return x.DhcpEnable
	}
	return false
}

func (x *CreateSubnetRequest) GetPrimaryDns() string {
	if x != nil {
		return x.PrimaryDns
	}
	return ""
}

func (x *CreateSubnetRequest) GetSecondaryDns() string {
	if x != nil {
		return x.SecondaryDns
	}
	return ""
}

func (x *CreateSubnetRequest) GetDnsList() []string {
	if x != nil {
		return x.DnsList
	}
	return nil
}

func (x *CreateSubnetRequest) GetAvailabilityZone() string {
	if x != nil {
		return x.AvailabilityZone
	}
	return ""
}

func (x *CreateSubnetRequest) GetExtraDhcpOpts() []*ExtraDhcpOpts {
	if x != nil {
		return x.ExtraDhcpOpts
	}
	return nil
}

type CreateSubnetResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Subnet *Subnet `protobuf:"bytes,1,opt,name=subnet,proto3" json:"subnet,omitempty"`
}

func (x *CreateSubnetResponse) Reset() {
	*x = CreateSubnetResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[34]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateSubnetResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateSubnetResponse) ProtoMessage() {}

func (x *CreateSubnetResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[34]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateSubnetResponse.ProtoReflect.Descriptor instead.
func (*CreateSubnetResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{34}
}

func (x *CreateSubnetResponse) GetSubnet() *Subnet {
	if x != nil {
		return x.Subnet
	}
	return nil
}

type ListSubnetsRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	RegionId  string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	// 按照vpc_id过滤查询, 企业项目细粒度授权场景下，该字段必传
	VpcId string `protobuf:"bytes,5,opt,name=vpc_id,json=vpcId,proto3" json:"vpc_id,omitempty"`
}

func (x *ListSubnetsRequest) Reset() {
	*x = ListSubnetsRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[35]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListSubnetsRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListSubnetsRequest) ProtoMessage() {}

func (x *ListSubnetsRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[35]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListSubnetsRequest.ProtoReflect.Descriptor instead.
func (*ListSubnetsRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{35}
}

func (x *ListSubnetsRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *ListSubnetsRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *ListSubnetsRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *ListSubnetsRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *ListSubnetsRequest) GetVpcId() string {
	if x != nil {
		return x.VpcId
	}
	return ""
}

type ListSubnetsResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Subnets []*Subnet `protobuf:"bytes,1,rep,name=subnets,proto3" json:"subnets,omitempty"`
}

func (x *ListSubnetsResponse) Reset() {
	*x = ListSubnetsResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[36]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListSubnetsResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListSubnetsResponse) ProtoMessage() {}

func (x *ListSubnetsResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[36]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListSubnetsResponse.ProtoReflect.Descriptor instead.
func (*ListSubnetsResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{36}
}

func (x *ListSubnetsResponse) GetSubnets() []*Subnet {
	if x != nil {
		return x.Subnets
	}
	return nil
}

type UpdateSubnetRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	RegionId  string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	// 子网对应的vpc_id
	VpcId string `protobuf:"bytes,5,opt,name=vpc_id,json=vpcId,proto3" json:"vpc_id,omitempty"`
	// 子网唯一标识
	SubnetId string `protobuf:"bytes,6,opt,name=subnet_id,json=subnetId,proto3" json:"subnet_id,omitempty"`
	// 功能说明：子网名称 取值范围：1-64，支持数字、字母、中文、_(下划线)、-（中划线）、.（点）
	Name string `protobuf:"bytes,7,opt,name=name,proto3" json:"name,omitempty"`
	// 功能说明：子网描述 取值范围：0-255个字符，不能包含“<”和“>”。
	Description string `protobuf:"bytes,8,opt,name=description,proto3" json:"description,omitempty"`
	// 功能说明：是否创建ipv6子网 取值范围：true（开启），false（关闭）
	Ipv6Enable bool `protobuf:"varint,9,opt,name=ipv6_enable,json=ipv6Enable,proto3" json:"ipv6_enable,omitempty"`
	// 功能说明：子网是否开启dhcp功能 取值范围：true（开启），false（关闭） 约束：不填时默认为true。当设置为false时，会导致新创建的ECS无法获取IP地址，cloudinit无法注入账号密码，请谨慎操作。
	DhcpEnable bool `protobuf:"varint,10,opt,name=dhcp_enable,json=dhcpEnable,proto3" json:"dhcp_enable,omitempty"`
	// 功能说明：子网dns服务器地址1 约束：ip格式 默认值：不填时为空 [内网DNS地址请参见](https://support.huaweicloud.com/dns_faq/dns_faq_002.html) [通过API获取请参见](https://support.huaweicloud.com/api-dns/dns_api_69001.html)
	PrimaryDns string `protobuf:"bytes,11,opt,name=primary_dns,json=primaryDns,proto3" json:"primary_dns,omitempty"`
	// 功能说明：子网dns服务器地址2 约束：ip格式 默认值：不填时为空 [内网DNS地址请参见](https://support.huaweicloud.com/dns_faq/dns_faq_002.html) [通过API获取请参见](https://support.huaweicloud.com/api-dns/dns_api_69001.html)
	SecondaryDns string `protobuf:"bytes,12,opt,name=secondary_dns,json=secondaryDns,proto3" json:"secondary_dns,omitempty"`
	// 功能说明：子网dns服务器地址的集合；如果想使用两个以上dns服务器，请使用该字段。 约束：是子网dns服务器地址1跟子网dns服务器地址2的合集的父集 默认值：不填时为空，无法使用云内网DNS功能 [内网DNS地址请参见](https://support.huaweicloud.com/dns_faq/dns_faq_002.html) [通过API获取请参见](https://support.huaweicloud.com/api-dns/dns_api_69001.html)
	DnsList []string `protobuf:"bytes,13,rep,name=dns_list,json=dnsList,proto3" json:"dns_list,omitempty"`
	// 子网配置的NTP地址
	ExtraDhcpOpts []*ExtraDhcpOpts `protobuf:"bytes,14,rep,name=extra_dhcp_opts,json=extraDhcpOpts,proto3" json:"extra_dhcp_opts,omitempty"`
}

func (x *UpdateSubnetRequest) Reset() {
	*x = UpdateSubnetRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[37]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateSubnetRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateSubnetRequest) ProtoMessage() {}

func (x *UpdateSubnetRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[37]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateSubnetRequest.ProtoReflect.Descriptor instead.
func (*UpdateSubnetRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{37}
}

func (x *UpdateSubnetRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *UpdateSubnetRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *UpdateSubnetRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *UpdateSubnetRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *UpdateSubnetRequest) GetVpcId() string {
	if x != nil {
		return x.VpcId
	}
	return ""
}

func (x *UpdateSubnetRequest) GetSubnetId() string {
	if x != nil {
		return x.SubnetId
	}
	return ""
}

func (x *UpdateSubnetRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *UpdateSubnetRequest) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *UpdateSubnetRequest) GetIpv6Enable() bool {
	if x != nil {
		return x.Ipv6Enable
	}
	return false
}

func (x *UpdateSubnetRequest) GetDhcpEnable() bool {
	if x != nil {
		return x.DhcpEnable
	}
	return false
}

func (x *UpdateSubnetRequest) GetPrimaryDns() string {
	if x != nil {
		return x.PrimaryDns
	}
	return ""
}

func (x *UpdateSubnetRequest) GetSecondaryDns() string {
	if x != nil {
		return x.SecondaryDns
	}
	return ""
}

func (x *UpdateSubnetRequest) GetDnsList() []string {
	if x != nil {
		return x.DnsList
	}
	return nil
}

func (x *UpdateSubnetRequest) GetExtraDhcpOpts() []*ExtraDhcpOpts {
	if x != nil {
		return x.ExtraDhcpOpts
	}
	return nil
}

type UpdateSubnetResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// uuid形式的一个资源标识。
	SubnetId string `protobuf:"bytes,1,opt,name=subnet_id,json=subnetId,proto3" json:"subnet_id,omitempty"`
	// 功能说明：子网的状态。   取值范围：ACTIVE,UNKNOWN,ERROR       ACTIVE表示子网已挂载到ROUTER上       UNKNOWN表示子网还未挂载到ROUTER上       ERROR表示子网状态故障
	Status string `protobuf:"bytes,2,opt,name=status,proto3" json:"status,omitempty"`
}

func (x *UpdateSubnetResponse) Reset() {
	*x = UpdateSubnetResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[38]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *UpdateSubnetResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateSubnetResponse) ProtoMessage() {}

func (x *UpdateSubnetResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[38]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateSubnetResponse.ProtoReflect.Descriptor instead.
func (*UpdateSubnetResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{38}
}

func (x *UpdateSubnetResponse) GetSubnetId() string {
	if x != nil {
		return x.SubnetId
	}
	return ""
}

func (x *UpdateSubnetResponse) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

type DeleteSubnetRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	RegionId  string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	// 子网对应的vpc_id
	VpcId string `protobuf:"bytes,5,opt,name=vpc_id,json=vpcId,proto3" json:"vpc_id,omitempty"`
	// 子网ID
	SubnetId string `protobuf:"bytes,6,opt,name=subnet_id,json=subnetId,proto3" json:"subnet_id,omitempty"`
}

func (x *DeleteSubnetRequest) Reset() {
	*x = DeleteSubnetRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[39]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *DeleteSubnetRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteSubnetRequest) ProtoMessage() {}

func (x *DeleteSubnetRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[39]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteSubnetRequest.ProtoReflect.Descriptor instead.
func (*DeleteSubnetRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{39}
}

func (x *DeleteSubnetRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *DeleteSubnetRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *DeleteSubnetRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *DeleteSubnetRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *DeleteSubnetRequest) GetVpcId() string {
	if x != nil {
		return x.VpcId
	}
	return ""
}

func (x *DeleteSubnetRequest) GetSubnetId() string {
	if x != nil {
		return x.SubnetId
	}
	return ""
}

type DeleteSubnetResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields
}

func (x *DeleteSubnetResponse) Reset() {
	*x = DeleteSubnetResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[40]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *DeleteSubnetResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DeleteSubnetResponse) ProtoMessage() {}

func (x *DeleteSubnetResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[40]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DeleteSubnetResponse.ProtoReflect.Descriptor instead.
func (*DeleteSubnetResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{40}
}

type ShowSubnetRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	RegionId  string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	// 子网ID
	SubnetId string `protobuf:"bytes,5,opt,name=subnet_id,json=subnetId,proto3" json:"subnet_id,omitempty"`
}

func (x *ShowSubnetRequest) Reset() {
	*x = ShowSubnetRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[41]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ShowSubnetRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ShowSubnetRequest) ProtoMessage() {}

func (x *ShowSubnetRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[41]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ShowSubnetRequest.ProtoReflect.Descriptor instead.
func (*ShowSubnetRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{41}
}

func (x *ShowSubnetRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *ShowSubnetRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *ShowSubnetRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *ShowSubnetRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *ShowSubnetRequest) GetSubnetId() string {
	if x != nil {
		return x.SubnetId
	}
	return ""
}

type ShowSubnetResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	Subnet *Subnet `protobuf:"bytes,1,opt,name=subnet,proto3" json:"subnet,omitempty"`
}

func (x *ShowSubnetResponse) Reset() {
	*x = ShowSubnetResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[42]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ShowSubnetResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ShowSubnetResponse) ProtoMessage() {}

func (x *ShowSubnetResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[42]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ShowSubnetResponse.ProtoReflect.Descriptor instead.
func (*ShowSubnetResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{42}
}

func (x *ShowSubnetResponse) GetSubnet() *Subnet {
	if x != nil {
		return x.Subnet
	}
	return nil
}

type ShowJobRequest struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	AccessKey string `protobuf:"bytes,1,opt,name=access_key,json=accessKey,proto3" json:"access_key,omitempty"`
	SecretKey string `protobuf:"bytes,2,opt,name=secret_key,json=secretKey,proto3" json:"secret_key,omitempty"`
	RegionId  string `protobuf:"bytes,3,opt,name=region_id,json=regionId,proto3" json:"region_id,omitempty"`
	ProjectId string `protobuf:"bytes,4,opt,name=project_id,json=projectId,proto3" json:"project_id,omitempty"`
	// 异步请求的任务ID
	JobId string `protobuf:"bytes,5,opt,name=job_id,json=jobId,proto3" json:"job_id,omitempty"`
}

func (x *ShowJobRequest) Reset() {
	*x = ShowJobRequest{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[43]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ShowJobRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ShowJobRequest) ProtoMessage() {}

func (x *ShowJobRequest) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[43]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ShowJobRequest.ProtoReflect.Descriptor instead.
func (*ShowJobRequest) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{43}
}

func (x *ShowJobRequest) GetAccessKey() string {
	if x != nil {
		return x.AccessKey
	}
	return ""
}

func (x *ShowJobRequest) GetSecretKey() string {
	if x != nil {
		return x.SecretKey
	}
	return ""
}

func (x *ShowJobRequest) GetRegionId() string {
	if x != nil {
		return x.RegionId
	}
	return ""
}

func (x *ShowJobRequest) GetProjectId() string {
	if x != nil {
		return x.ProjectId
	}
	return ""
}

func (x *ShowJobRequest) GetJobId() string {
	if x != nil {
		return x.JobId
	}
	return ""
}

type ShowJobResponse struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 开始时间。
	BeginTime string `protobuf:"bytes,1,opt,name=begin_time,json=beginTime,proto3" json:"begin_time,omitempty"`
	// 查询Job的API请求出现错误时，返回的错误码。
	Code string `protobuf:"bytes,2,opt,name=code,proto3" json:"code,omitempty"`
	// 结束时间。
	EndTime string `protobuf:"bytes,3,opt,name=end_time,json=endTime,proto3" json:"end_time,omitempty"`
	// Job执行失败时的错误码。  Job执行成功后，该值为空。
	ErrorCode string `protobuf:"bytes,4,opt,name=error_code,json=errorCode,proto3" json:"error_code,omitempty"`
	// Job执行失败时的错误原因。  Job执行成功后，该值为空。
	FailReason string `protobuf:"bytes,5,opt,name=fail_reason,json=failReason,proto3" json:"fail_reason,omitempty"`
	// 异步请求的任务ID。
	JobId string `protobuf:"bytes,6,opt,name=job_id,json=jobId,proto3" json:"job_id,omitempty"`
	// 异步请求的任务类型。
	JobType string `protobuf:"bytes,7,opt,name=job_type,json=jobType,proto3" json:"job_type,omitempty"`
	// 查询Job的API请求出现错误时，返回的错误消息。
	Message string `protobuf:"bytes,8,opt,name=message,proto3" json:"message,omitempty"`
	// Job的状态。  - SUCCESS：成功。  - RUNNING：运行中。  - FAIL：失败。  - INIT：正在初始化。
	Status string `protobuf:"bytes,9,opt,name=status,proto3" json:"status,omitempty"`
}

func (x *ShowJobResponse) Reset() {
	*x = ShowJobResponse{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[44]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ShowJobResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ShowJobResponse) ProtoMessage() {}

func (x *ShowJobResponse) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[44]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ShowJobResponse.ProtoReflect.Descriptor instead.
func (*ShowJobResponse) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{44}
}

func (x *ShowJobResponse) GetBeginTime() string {
	if x != nil {
		return x.BeginTime
	}
	return ""
}

func (x *ShowJobResponse) GetCode() string {
	if x != nil {
		return x.Code
	}
	return ""
}

func (x *ShowJobResponse) GetEndTime() string {
	if x != nil {
		return x.EndTime
	}
	return ""
}

func (x *ShowJobResponse) GetErrorCode() string {
	if x != nil {
		return x.ErrorCode
	}
	return ""
}

func (x *ShowJobResponse) GetFailReason() string {
	if x != nil {
		return x.FailReason
	}
	return ""
}

func (x *ShowJobResponse) GetJobId() string {
	if x != nil {
		return x.JobId
	}
	return ""
}

func (x *ShowJobResponse) GetJobType() string {
	if x != nil {
		return x.JobType
	}
	return ""
}

func (x *ShowJobResponse) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *ShowJobResponse) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

// 待创建云服务器的网卡信息。  约束：  - 网卡对应的子网（subnet）必须属于vpcid对应的VPC。 - 当前单个云服务器支持最多挂载12张网卡。 - 不同的规格对网卡上限有一定的区别，参考[规格清单](https://support.huaweicloud.com/productdesc-ecs/zh-cn_topic_0159822360.html)。
type CreateInstancesRequest_Nic struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 待创建云服务器所在的子网信息,需要指定vpcid对应VPC下的子网ID,UUID格式。
	SubnetId string `protobuf:"bytes,1,opt,name=subnet_id,json=subnetId,proto3" json:"subnet_id,omitempty"`
}

func (x *CreateInstancesRequest_Nic) Reset() {
	*x = CreateInstancesRequest_Nic{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[45]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateInstancesRequest_Nic) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateInstancesRequest_Nic) ProtoMessage() {}

func (x *CreateInstancesRequest_Nic) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[45]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateInstancesRequest_Nic.ProtoReflect.Descriptor instead.
func (*CreateInstancesRequest_Nic) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{0, 0}
}

func (x *CreateInstancesRequest_Nic) GetSubnetId() string {
	if x != nil {
		return x.SubnetId
	}
	return ""
}

type CreateInstancesRequest_RootVolume struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 云服务器系统盘对应的磁盘类型，需要与系统所提供的磁盘类型相匹配。  - SATA：普通IO磁盘类型。 - SAS：高IO磁盘类型。 - SSD：超高IO磁盘类型。 - co-p1：高IO (性能优化Ⅰ型) - uh-l1：超高IO (时延优化)  > 说明： >  > 对于HANA云服务器、HL1型云服务器、HL2型云服务器，需使用co-p1和uh-l1两种磁盘类型。对于其他类型的云服务器，不能使用co-p1和uh-l1两种磁盘类型。
	VolumeType string `protobuf:"bytes,1,opt,name=volume_type,json=volumeType,proto3" json:"volume_type,omitempty"`
	// 系统盘大小，容量单位为GB， 输入大小范围为[1,1024]。  约束：  - 系统盘大小取值应不小于镜像支持的系统盘的最小值(镜像的min_disk属性)。 - 若该参数没有指定或者指定为0时，系统盘大小默认取值为镜像中系统盘的最小值(镜像的min_disk属性)。  > 说明：  > 镜像系统盘的最小值(镜像的min_disk属性)可在控制台中点击镜像详情查看。或通过调用“查询镜像详情（OpenStack原生）”API获取，详细操作请参考[《镜像服务API参考》](https://support.huaweicloud.com/api-ims/ims_03_0702.html)中“查询镜像详情（OpenStack原生）”章节。
	Size int32 `protobuf:"varint,2,opt,name=size,proto3" json:"size,omitempty"`
}

func (x *CreateInstancesRequest_RootVolume) Reset() {
	*x = CreateInstancesRequest_RootVolume{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[46]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *CreateInstancesRequest_RootVolume) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*CreateInstancesRequest_RootVolume) ProtoMessage() {}

func (x *CreateInstancesRequest_RootVolume) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[46]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use CreateInstancesRequest_RootVolume.ProtoReflect.Descriptor instead.
func (*CreateInstancesRequest_RootVolume) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{0, 1}
}

func (x *CreateInstancesRequest_RootVolume) GetVolumeType() string {
	if x != nil {
		return x.VolumeType
	}
	return ""
}

func (x *CreateInstancesRequest_RootVolume) GetSize() int32 {
	if x != nil {
		return x.Size
	}
	return 0
}

type ListInstancesResponse_Instance struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 弹性云服务器状态。  取值范围：  ACTIVE、BUILD、DELETED、ERROR、HARD_REBOOT、MIGRATING、PAUSED、REBOOT、REBUILD、RESIZE、REVERT_RESIZE、SHUTOFF、SHELVED、SHELVED_OFFLOADED、SOFT_DELETED、SUSPENDED、VERIFY_RESIZE  弹性云服务器状态说明请参考[云服务器状态](https://support.huaweicloud.com/api-ecs/ecs_08_0002.html)
	Status string `protobuf:"bytes,1,opt,name=status,proto3" json:"status,omitempty"`
	// 弹性云服务器更新时间。  时间格式例如：2019-05-22T03:30:52Z
	Updated string `protobuf:"bytes,2,opt,name=updated,proto3" json:"updated,omitempty"`
	// 弹性云服务器自动释放时间。  时间格式例如：2020-01-19T03:30:52Z
	AutoTerminateTime string `protobuf:"bytes,3,opt,name=auto_terminate_time,json=autoTerminateTime,proto3" json:"auto_terminate_time,omitempty"`
	// 弹性云服务器所在主机的主机ID。
	HostId string `protobuf:"bytes,4,opt,name=host_id,json=hostId,proto3" json:"host_id,omitempty"`
	// 弹性云服务器所在主机的主机名称。
	OsExtSrvAttrHost string                                                     `protobuf:"bytes,5,opt,name=os_ext_srv_attr_host,json=osExtSrvAttrHost,proto3" json:"os_ext_srv_attr_host,omitempty"`
	Addresses        map[string]*ListInstancesResponse_Instance_ServerAddresses `protobuf:"bytes,6,rep,name=addresses,proto3" json:"addresses,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
	// 弹性云服务器使用的密钥对名称。
	KeyName string `protobuf:"bytes,7,opt,name=key_name,json=keyName,proto3" json:"key_name,omitempty"`
	// 镜像id
	ImageId string `protobuf:"bytes,8,opt,name=image_id,json=imageId,proto3" json:"image_id,omitempty"`
	// 扩展属性，弹性云服务器当前任务的状态。  取值范围请参考[云服务器状态](https://support.huaweicloud.com/api-ecs/ecs_08_0002.html)表3。
	OsExtStsTaskState string `protobuf:"bytes,9,opt,name=os_ext_sts_task_state,json=osExtStsTaskState,proto3" json:"os_ext_sts_task_state,omitempty"`
	// 扩展属性，弹性云服务器当前状态。  云服务器状态说明请参考[云服务器状态](https://support.huaweicloud.com/api-ecs/ecs_08_0002.html)。
	OsExtStsVmState string `protobuf:"bytes,10,opt,name=os_ext_sts_vm_state,json=osExtStsVmState,proto3" json:"os_ext_sts_vm_state,omitempty"`
	// 扩展属性，弹性云服务器别名。
	OsExtSrvAttrInstanceName string `protobuf:"bytes,11,opt,name=os_ext_srv_attr_instance_name,json=osExtSrvAttrInstanceName,proto3" json:"os_ext_srv_attr_instance_name,omitempty"`
	// 扩展属性，弹性云服务器所在虚拟化主机名。
	OsExtSrvAttrHypervisorHostname string                                       `protobuf:"bytes,12,opt,name=os_ext_srv_attr_hypervisor_hostname,json=osExtSrvAttrHypervisorHostname,proto3" json:"os_ext_srv_attr_hypervisor_hostname,omitempty"`
	InstanceType                   *ListInstancesResponse_Instance_InstanceType `protobuf:"bytes,13,opt,name=instance_type,json=instanceType,proto3" json:"instance_type,omitempty"`
	// 弹性云服务器ID，格式为UUID。
	Id             string                                          `protobuf:"bytes,14,opt,name=id,proto3" json:"id,omitempty"`
	SecurityGroups []*ListInstancesResponse_Instance_SecurityGroup `protobuf:"bytes,15,rep,name=security_groups,json=securityGroups,proto3" json:"security_groups,omitempty"`
	// 扩展属性，弹性云服务器所在可用区名称。
	OsExtAzAvailabilityZone string `protobuf:"bytes,16,opt,name=os_ext_az_availability_zone,json=osExtAzAvailabilityZone,proto3" json:"os_ext_az_availability_zone,omitempty"`
	// 创建弹性云服务器的用户ID，格式为UUID。
	UserId string `protobuf:"bytes,17,opt,name=user_id,json=userId,proto3" json:"user_id,omitempty"`
	// 弹性云服务器名称。
	Name string `protobuf:"bytes,18,opt,name=name,proto3" json:"name,omitempty"`
	// 弹性云服务器创建时间。  时间格式例如：2019-05-22T03:19:19Z
	Created string `protobuf:"bytes,19,opt,name=created,proto3" json:"created,omitempty"`
	// 弹性云服务器所属租户ID，即项目id，和project_id表示相同的概念，格式为UUID。
	TenantId string `protobuf:"bytes,20,opt,name=tenant_id,json=tenantId,proto3" json:"tenant_id,omitempty"`
	// 扩展属性， diskConfig的类型。  - MANUAL，镜像空间不会扩展。 - AUTO，系统盘镜像空间会自动扩展为与flavor大小一致。
	OsDcfDiskConfig string `protobuf:"bytes,21,opt,name=os_dcf_disk_config,json=osDcfDiskConfig,proto3" json:"os_dcf_disk_config,omitempty"`
	// 预留属性。
	AccessIpv4 string `protobuf:"bytes,22,opt,name=access_ipv4,json=accessIpv4,proto3" json:"access_ipv4,omitempty"`
	// 预留属性。
	AccessIpv6 string                                      `protobuf:"bytes,23,opt,name=access_ipv6,json=accessIpv6,proto3" json:"access_ipv6,omitempty"`
	Fault      *ListInstancesResponse_Instance_ServerFault `protobuf:"bytes,24,opt,name=fault,proto3" json:"fault,omitempty"`
	// 弹性云服务器进度。
	Progress int32 `protobuf:"varint,25,opt,name=progress,proto3" json:"progress,omitempty"`
	// 扩展属性，弹性云服务器电源状态。
	OsExtStsPowerState int32 `protobuf:"varint,26,opt,name=os_ext_sts_power_state,json=osExtStsPowerState,proto3" json:"os_ext_sts_power_state,omitempty"`
	// config drive信息。
	ConfigDrive string `protobuf:"bytes,27,opt,name=config_drive,json=configDrive,proto3" json:"config_drive,omitempty"`
	// 弹性云服务器元数据。  > 说明： >  > 元数据包含系统默认添加字段和用户设置的字段。  系统默认添加字段  1. charging_mode 云服务器的计费类型。  - “0”：按需计费（即postPaid-后付费方式）。 - “1”：按包年包月计费（即prePaid-预付费方式）。\"2\"：竞价实例计费  2. metering.order_id 按“包年/包月”计费的云服务器对应的订单ID。  3. metering.product_id 按“包年/包月”计费的云服务器对应的产品ID。  4. vpc_id 云服务器所属的虚拟私有云ID。  5. EcmResStatus 云服务器的冻结状态。  - normal：云服务器正常状态（未被冻结）。 - freeze：云服务器被冻结。  > 当云服务器被冻结或者解冻后，系统默认添加该字段，且该字段必选。  6. metering.image_id 云服务器操作系统对应的镜像ID  7.  metering.imagetype 镜像类型，目前支持：  - 公共镜像（gold） - 私有镜像（private） - 共享镜像（shared）  8. metering.resourcespeccode 云服务器对应的资源规格。  9. image_name 云服务器操作系统对应的镜像名称。  10. os_bit 操作系统位数，一般取值为“32”或者“64”。  11. lockCheckEndpoint 回调URL，用于检查弹性云服务器的加锁是否有效。  - 如果有效，则云服务器保持锁定状态。 - 如果无效，解除锁定状态，删除失效的锁。  12. lockSource 弹性云服务器来自哪个服务。订单加锁（ORDER）  13. lockSourceId 弹性云服务器的加锁来自哪个ID。lockSource为“ORDER”时，lockSourceId为订单ID。  14. lockScene 弹性云服务器的加锁类型。  - 按需转包周期（TO_PERIOD_LOCK）  15. virtual_env_type  - IOS镜像创建虚拟机，\"virtual_env_type\": \"IsoImage\" 属性； - 非IOS镜像创建虚拟机，在19.5.0版本以后创建的虚拟机将不会添加virtual_env_type 属性，而在此之前的版本创建的虚拟机可能会返回\"virtual_env_type\": \"FusionCompute\"属性 。  > virtual_env_type属性不允许用户增加、删除和修改。  16. metering.resourcetype 云服务器对应的资源类型。  17. os_type 操作系统类型，取值为：Linux、Windows。  18. cascaded.instance_extrainfo 系统内部虚拟机扩展信息。  19. __support_agent_list 云服务器是否支持企业主机安全、主机监控。  - “hss”：企业主机安全 -  “ces”：主机监控  20. agency_name 委托的名称。  委托是由租户管理员在统一身份认证服务（Identity and Access Management，IAM）上创建的，可以为弹性云服务器提供访问云服务的临时凭证。
	Metadata map[string]string `protobuf:"bytes,28,rep,name=metadata,proto3" json:"metadata,omitempty" protobuf_key:"bytes,1,opt,name=key,proto3" protobuf_val:"bytes,2,opt,name=value,proto3"`
	// 弹性云服务器启动时间。时间格式例如：2019-05-22T03:23:59.000000
	OsSrvUsgLaunchedAt string `protobuf:"bytes,29,opt,name=os_srv_usg_launched_at,json=osSrvUsgLaunchedAt,proto3" json:"os_srv_usg_launched_at,omitempty"`
	// 弹性云服务器删除时间。  时间格式例如：2019-05-22T03:23:59.000000
	OsSrvUsgTerminatedAt string `protobuf:"bytes,30,opt,name=os_srv_usg_terminated_at,json=osSrvUsgTerminatedAt,proto3" json:"os_srv_usg_terminated_at,omitempty"`
	// 挂载到弹性云服务器上的磁盘。
	OsExtendedVolumesVolumes_Attached []*ListInstancesResponse_Instance_ServerExtendVolumeAttachment `protobuf:"bytes,31,rep,name=os_extended_volumes_volumes_Attached,json=osExtendedVolumesVolumesAttached,proto3" json:"os_extended_volumes_volumes_Attached,omitempty"`
	// 弹性云服务器的描述信息。
	Description string `protobuf:"bytes,32,opt,name=description,proto3" json:"description,omitempty"`
	// nova-compute状态。  - UP：服务正常 - UNKNOWN：状态未知 - DOWN：服务异常 - MAINTENANCE：维护状态 - 空字符串：弹性云服务器无主机信息
	HostStatus string `protobuf:"bytes,33,opt,name=host_status,json=hostStatus,proto3" json:"host_status,omitempty"`
	// 弹性云服务器的主机名。
	OsExtSrvAttrHostname string `protobuf:"bytes,34,opt,name=os_ext_srv_attr_hostname,json=osExtSrvAttrHostname,proto3" json:"os_ext_srv_attr_hostname,omitempty"`
	// 批量创建场景，弹性云服务器的预留ID。
	OsExtSrvAttrReservationId string `protobuf:"bytes,35,opt,name=os_ext_srv_attr_reservation_id,json=osExtSrvAttrReservationId,proto3" json:"os_ext_srv_attr_reservation_id,omitempty"`
	// 批量创建场景，弹性云服务器的启动顺序。
	OsExtSrvAttrLaunchIndex int32 `protobuf:"varint,36,opt,name=os_ext_srv_attr_launch_index,json=osExtSrvAttrLaunchIndex,proto3" json:"os_ext_srv_attr_launch_index,omitempty"`
	// 若使用AMI格式的镜像，则表示kernel image的UUID；否则，留空。
	OsExtSrvAttrKernelId string `protobuf:"bytes,37,opt,name=os_ext_srv_attr_kernel_id,json=osExtSrvAttrKernelId,proto3" json:"os_ext_srv_attr_kernel_id,omitempty"`
	// 若使用AMI格式镜像，则表示ramdisk image的UUID；否则，留空。
	OsExtSrvAttrRamdiskId string `protobuf:"bytes,38,opt,name=os_ext_srv_attr_ramdisk_id,json=osExtSrvAttrRamdiskId,proto3" json:"os_ext_srv_attr_ramdisk_id,omitempty"`
	// 弹性云服务器系统盘的设备名称。
	OsExtSrvAttrRootDeviceName string `protobuf:"bytes,39,opt,name=os_ext_srv_attr_root_device_name,json=osExtSrvAttrRootDeviceName,proto3" json:"os_ext_srv_attr_root_device_name,omitempty"`
	// 创建弹性云服务器时指定的user_data。
	OsExtSrvAttrUserData string `protobuf:"bytes,40,opt,name=os_ext_srv_attr_user_data,json=osExtSrvAttrUserData,proto3" json:"os_ext_srv_attr_user_data,omitempty"`
	// 弹性云服务器是否为锁定状态。  - true：锁定 - false：未锁定
	Locked bool `protobuf:"varint,41,opt,name=locked,proto3" json:"locked,omitempty"`
	// 弹性云服务器标签。
	Tags             []string                                             `protobuf:"bytes,42,rep,name=tags,proto3" json:"tags,omitempty"`
	OsSchedulerHints *ListInstancesResponse_Instance_ServerSchedulerHints `protobuf:"bytes,43,opt,name=os_scheduler_hints,json=osSchedulerHints,proto3" json:"os_scheduler_hints,omitempty"`
	// 弹性云服务器所属的企业项目ID。
	EnterpriseProjectId string `protobuf:"bytes,44,opt,name=enterprise_project_id,json=enterpriseProjectId,proto3" json:"enterprise_project_id,omitempty"`
	// 弹性云服务器系统标签。
	SysTags    []*ListInstancesResponse_Instance_ServerSystemTag `protobuf:"bytes,45,rep,name=sys_tags,json=sysTags,proto3" json:"sys_tags,omitempty"`
	CpuOptions *ListInstancesResponse_Instance_CpuOptions        `protobuf:"bytes,46,opt,name=cpu_options,json=cpuOptions,proto3" json:"cpu_options,omitempty"`
	Hypervisor *ListInstancesResponse_Instance_Hypervisor        `protobuf:"bytes,47,opt,name=hypervisor,proto3" json:"hypervisor,omitempty"`
}

func (x *ListInstancesResponse_Instance) Reset() {
	*x = ListInstancesResponse_Instance{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[47]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstancesResponse_Instance) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstancesResponse_Instance) ProtoMessage() {}

func (x *ListInstancesResponse_Instance) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[47]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstancesResponse_Instance.ProtoReflect.Descriptor instead.
func (*ListInstancesResponse_Instance) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{3, 0}
}

func (x *ListInstancesResponse_Instance) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetUpdated() string {
	if x != nil {
		return x.Updated
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetAutoTerminateTime() string {
	if x != nil {
		return x.AutoTerminateTime
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetHostId() string {
	if x != nil {
		return x.HostId
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetOsExtSrvAttrHost() string {
	if x != nil {
		return x.OsExtSrvAttrHost
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetAddresses() map[string]*ListInstancesResponse_Instance_ServerAddresses {
	if x != nil {
		return x.Addresses
	}
	return nil
}

func (x *ListInstancesResponse_Instance) GetKeyName() string {
	if x != nil {
		return x.KeyName
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetImageId() string {
	if x != nil {
		return x.ImageId
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetOsExtStsTaskState() string {
	if x != nil {
		return x.OsExtStsTaskState
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetOsExtStsVmState() string {
	if x != nil {
		return x.OsExtStsVmState
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetOsExtSrvAttrInstanceName() string {
	if x != nil {
		return x.OsExtSrvAttrInstanceName
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetOsExtSrvAttrHypervisorHostname() string {
	if x != nil {
		return x.OsExtSrvAttrHypervisorHostname
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetInstanceType() *ListInstancesResponse_Instance_InstanceType {
	if x != nil {
		return x.InstanceType
	}
	return nil
}

func (x *ListInstancesResponse_Instance) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetSecurityGroups() []*ListInstancesResponse_Instance_SecurityGroup {
	if x != nil {
		return x.SecurityGroups
	}
	return nil
}

func (x *ListInstancesResponse_Instance) GetOsExtAzAvailabilityZone() string {
	if x != nil {
		return x.OsExtAzAvailabilityZone
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetUserId() string {
	if x != nil {
		return x.UserId
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetCreated() string {
	if x != nil {
		return x.Created
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetTenantId() string {
	if x != nil {
		return x.TenantId
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetOsDcfDiskConfig() string {
	if x != nil {
		return x.OsDcfDiskConfig
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetAccessIpv4() string {
	if x != nil {
		return x.AccessIpv4
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetAccessIpv6() string {
	if x != nil {
		return x.AccessIpv6
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetFault() *ListInstancesResponse_Instance_ServerFault {
	if x != nil {
		return x.Fault
	}
	return nil
}

func (x *ListInstancesResponse_Instance) GetProgress() int32 {
	if x != nil {
		return x.Progress
	}
	return 0
}

func (x *ListInstancesResponse_Instance) GetOsExtStsPowerState() int32 {
	if x != nil {
		return x.OsExtStsPowerState
	}
	return 0
}

func (x *ListInstancesResponse_Instance) GetConfigDrive() string {
	if x != nil {
		return x.ConfigDrive
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetMetadata() map[string]string {
	if x != nil {
		return x.Metadata
	}
	return nil
}

func (x *ListInstancesResponse_Instance) GetOsSrvUsgLaunchedAt() string {
	if x != nil {
		return x.OsSrvUsgLaunchedAt
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetOsSrvUsgTerminatedAt() string {
	if x != nil {
		return x.OsSrvUsgTerminatedAt
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetOsExtendedVolumesVolumes_Attached() []*ListInstancesResponse_Instance_ServerExtendVolumeAttachment {
	if x != nil {
		return x.OsExtendedVolumesVolumes_Attached
	}
	return nil
}

func (x *ListInstancesResponse_Instance) GetDescription() string {
	if x != nil {
		return x.Description
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetHostStatus() string {
	if x != nil {
		return x.HostStatus
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetOsExtSrvAttrHostname() string {
	if x != nil {
		return x.OsExtSrvAttrHostname
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetOsExtSrvAttrReservationId() string {
	if x != nil {
		return x.OsExtSrvAttrReservationId
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetOsExtSrvAttrLaunchIndex() int32 {
	if x != nil {
		return x.OsExtSrvAttrLaunchIndex
	}
	return 0
}

func (x *ListInstancesResponse_Instance) GetOsExtSrvAttrKernelId() string {
	if x != nil {
		return x.OsExtSrvAttrKernelId
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetOsExtSrvAttrRamdiskId() string {
	if x != nil {
		return x.OsExtSrvAttrRamdiskId
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetOsExtSrvAttrRootDeviceName() string {
	if x != nil {
		return x.OsExtSrvAttrRootDeviceName
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetOsExtSrvAttrUserData() string {
	if x != nil {
		return x.OsExtSrvAttrUserData
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetLocked() bool {
	if x != nil {
		return x.Locked
	}
	return false
}

func (x *ListInstancesResponse_Instance) GetTags() []string {
	if x != nil {
		return x.Tags
	}
	return nil
}

func (x *ListInstancesResponse_Instance) GetOsSchedulerHints() *ListInstancesResponse_Instance_ServerSchedulerHints {
	if x != nil {
		return x.OsSchedulerHints
	}
	return nil
}

func (x *ListInstancesResponse_Instance) GetEnterpriseProjectId() string {
	if x != nil {
		return x.EnterpriseProjectId
	}
	return ""
}

func (x *ListInstancesResponse_Instance) GetSysTags() []*ListInstancesResponse_Instance_ServerSystemTag {
	if x != nil {
		return x.SysTags
	}
	return nil
}

func (x *ListInstancesResponse_Instance) GetCpuOptions() *ListInstancesResponse_Instance_CpuOptions {
	if x != nil {
		return x.CpuOptions
	}
	return nil
}

func (x *ListInstancesResponse_Instance) GetHypervisor() *ListInstancesResponse_Instance_Hypervisor {
	if x != nil {
		return x.Hypervisor
	}
	return nil
}

// 弹性云服务器的网络属性。
type ListInstancesResponse_Instance_ServerAddress struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// IP地址版本。  - “4”：代表IPv4。 - “6”：代表IPv6。
	Version string `protobuf:"bytes,1,opt,name=version,proto3" json:"version,omitempty"`
	// IP地址。
	Addr string `protobuf:"bytes,2,opt,name=addr,proto3" json:"addr,omitempty"`
	// IP地址类型。  - fixed：代表私有IP地址。 - floating：代表浮动IP地址。
	OsExtIpsType string `protobuf:"bytes,3,opt,name=os_ext_ips_type,json=osExtIpsType,proto3" json:"os_ext_ips_type,omitempty"`
	// MAC地址。
	OsExtIpsMacMacAddr string `protobuf:"bytes,4,opt,name=os_ext_ips_mac_mac_addr,json=osExtIpsMacMacAddr,proto3" json:"os_ext_ips_mac_mac_addr,omitempty"`
	// IP地址对应的端口ID。
	OsExtIpSportId string `protobuf:"bytes,5,opt,name=os_ext_ip_sport_id,json=osExtIpSportId,proto3" json:"os_ext_ip_sport_id,omitempty"`
}

func (x *ListInstancesResponse_Instance_ServerAddress) Reset() {
	*x = ListInstancesResponse_Instance_ServerAddress{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[48]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstancesResponse_Instance_ServerAddress) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstancesResponse_Instance_ServerAddress) ProtoMessage() {}

func (x *ListInstancesResponse_Instance_ServerAddress) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[48]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstancesResponse_Instance_ServerAddress.ProtoReflect.Descriptor instead.
func (*ListInstancesResponse_Instance_ServerAddress) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{3, 0, 0}
}

func (x *ListInstancesResponse_Instance_ServerAddress) GetVersion() string {
	if x != nil {
		return x.Version
	}
	return ""
}

func (x *ListInstancesResponse_Instance_ServerAddress) GetAddr() string {
	if x != nil {
		return x.Addr
	}
	return ""
}

func (x *ListInstancesResponse_Instance_ServerAddress) GetOsExtIpsType() string {
	if x != nil {
		return x.OsExtIpsType
	}
	return ""
}

func (x *ListInstancesResponse_Instance_ServerAddress) GetOsExtIpsMacMacAddr() string {
	if x != nil {
		return x.OsExtIpsMacMacAddr
	}
	return ""
}

func (x *ListInstancesResponse_Instance_ServerAddress) GetOsExtIpSportId() string {
	if x != nil {
		return x.OsExtIpSportId
	}
	return ""
}

type ListInstancesResponse_Instance_ServerAddresses struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ServerAddresses []*ListInstancesResponse_Instance_ServerAddress `protobuf:"bytes,1,rep,name=server_addresses,json=serverAddresses,proto3" json:"server_addresses,omitempty"`
}

func (x *ListInstancesResponse_Instance_ServerAddresses) Reset() {
	*x = ListInstancesResponse_Instance_ServerAddresses{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[49]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstancesResponse_Instance_ServerAddresses) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstancesResponse_Instance_ServerAddresses) ProtoMessage() {}

func (x *ListInstancesResponse_Instance_ServerAddresses) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[49]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstancesResponse_Instance_ServerAddresses.ProtoReflect.Descriptor instead.
func (*ListInstancesResponse_Instance_ServerAddresses) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{3, 0, 1}
}

func (x *ListInstancesResponse_Instance_ServerAddresses) GetServerAddresses() []*ListInstancesResponse_Instance_ServerAddress {
	if x != nil {
		return x.ServerAddresses
	}
	return nil
}

type ListInstancesResponse_Instance_InstanceType struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 弹性云服务器规格ID。
	Id string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	// 弹性云服务器规格名称。
	Name string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// 该云服务器规格对应要求系统盘大小，0为不限制。此字段在本系统中无效。
	Disk string `protobuf:"bytes,3,opt,name=disk,proto3" json:"disk,omitempty"`
	// 该云服务器规格对应的CPU核数。
	Vcpus string `protobuf:"bytes,4,opt,name=vcpus,proto3" json:"vcpus,omitempty"`
	// 该云服务器规格对应的内存大小，单位为MB。
	Ram string `protobuf:"bytes,5,opt,name=ram,proto3" json:"ram,omitempty"`
}

func (x *ListInstancesResponse_Instance_InstanceType) Reset() {
	*x = ListInstancesResponse_Instance_InstanceType{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[51]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstancesResponse_Instance_InstanceType) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstancesResponse_Instance_InstanceType) ProtoMessage() {}

func (x *ListInstancesResponse_Instance_InstanceType) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[51]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstancesResponse_Instance_InstanceType.ProtoReflect.Descriptor instead.
func (*ListInstancesResponse_Instance_InstanceType) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{3, 0, 3}
}

func (x *ListInstancesResponse_Instance_InstanceType) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *ListInstancesResponse_Instance_InstanceType) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *ListInstancesResponse_Instance_InstanceType) GetDisk() string {
	if x != nil {
		return x.Disk
	}
	return ""
}

func (x *ListInstancesResponse_Instance_InstanceType) GetVcpus() string {
	if x != nil {
		return x.Vcpus
	}
	return ""
}

func (x *ListInstancesResponse_Instance_InstanceType) GetRam() string {
	if x != nil {
		return x.Ram
	}
	return ""
}

// 弹性云服务器所属安全组列表。
type ListInstancesResponse_Instance_SecurityGroup struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 安全组名称或者UUID。
	Name string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// 安全组ID。
	Id string `protobuf:"bytes,2,opt,name=id,proto3" json:"id,omitempty"`
}

func (x *ListInstancesResponse_Instance_SecurityGroup) Reset() {
	*x = ListInstancesResponse_Instance_SecurityGroup{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[52]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstancesResponse_Instance_SecurityGroup) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstancesResponse_Instance_SecurityGroup) ProtoMessage() {}

func (x *ListInstancesResponse_Instance_SecurityGroup) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[52]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstancesResponse_Instance_SecurityGroup.ProtoReflect.Descriptor instead.
func (*ListInstancesResponse_Instance_SecurityGroup) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{3, 0, 4}
}

func (x *ListInstancesResponse_Instance_SecurityGroup) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *ListInstancesResponse_Instance_SecurityGroup) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

type ListInstancesResponse_Instance_ServerFault struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 错误码。
	Code int32 `protobuf:"varint,1,opt,name=code,proto3" json:"code,omitempty"`
	// 异常出现的时间。
	Created string `protobuf:"bytes,2,opt,name=created,proto3" json:"created,omitempty"`
	// 异常描述信息。
	Message string `protobuf:"bytes,3,opt,name=message,proto3" json:"message,omitempty"`
	// 异常详情信息。
	Details string `protobuf:"bytes,4,opt,name=details,proto3" json:"details,omitempty"`
}

func (x *ListInstancesResponse_Instance_ServerFault) Reset() {
	*x = ListInstancesResponse_Instance_ServerFault{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[53]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstancesResponse_Instance_ServerFault) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstancesResponse_Instance_ServerFault) ProtoMessage() {}

func (x *ListInstancesResponse_Instance_ServerFault) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[53]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstancesResponse_Instance_ServerFault.ProtoReflect.Descriptor instead.
func (*ListInstancesResponse_Instance_ServerFault) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{3, 0, 5}
}

func (x *ListInstancesResponse_Instance_ServerFault) GetCode() int32 {
	if x != nil {
		return x.Code
	}
	return 0
}

func (x *ListInstancesResponse_Instance_ServerFault) GetCreated() string {
	if x != nil {
		return x.Created
	}
	return ""
}

func (x *ListInstancesResponse_Instance_ServerFault) GetMessage() string {
	if x != nil {
		return x.Message
	}
	return ""
}

func (x *ListInstancesResponse_Instance_ServerFault) GetDetails() string {
	if x != nil {
		return x.Details
	}
	return ""
}

type ListInstancesResponse_Instance_ServerExtendVolumeAttachment struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 磁盘ID，格式为UUID。
	Id string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	// 删弹性云服务器时是否一并删除该磁盘。  - true：是 - false：否 微版本2.3及以上版本支持。
	DeleteOnTermination string `protobuf:"bytes,2,opt,name=delete_on_termination,json=deleteOnTermination,proto3" json:"delete_on_termination,omitempty"`
	// 启动标识，“0”代表启动盘，“-1“代表非启动盘。
	BootIndex string `protobuf:"bytes,3,opt,name=boot_index,json=bootIndex,proto3" json:"boot_index,omitempty"`
	// 云硬盘挂载盘符，即磁盘挂载点。
	Device string `protobuf:"bytes,4,opt,name=device,proto3" json:"device,omitempty"`
}

func (x *ListInstancesResponse_Instance_ServerExtendVolumeAttachment) Reset() {
	*x = ListInstancesResponse_Instance_ServerExtendVolumeAttachment{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[55]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstancesResponse_Instance_ServerExtendVolumeAttachment) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstancesResponse_Instance_ServerExtendVolumeAttachment) ProtoMessage() {}

func (x *ListInstancesResponse_Instance_ServerExtendVolumeAttachment) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[55]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstancesResponse_Instance_ServerExtendVolumeAttachment.ProtoReflect.Descriptor instead.
func (*ListInstancesResponse_Instance_ServerExtendVolumeAttachment) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{3, 0, 7}
}

func (x *ListInstancesResponse_Instance_ServerExtendVolumeAttachment) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *ListInstancesResponse_Instance_ServerExtendVolumeAttachment) GetDeleteOnTermination() string {
	if x != nil {
		return x.DeleteOnTermination
	}
	return ""
}

func (x *ListInstancesResponse_Instance_ServerExtendVolumeAttachment) GetBootIndex() string {
	if x != nil {
		return x.BootIndex
	}
	return ""
}

func (x *ListInstancesResponse_Instance_ServerExtendVolumeAttachment) GetDevice() string {
	if x != nil {
		return x.Device
	}
	return ""
}

type ListInstancesResponse_Instance_ServerSchedulerHints struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 反亲和性组信息。  UUID格式。
	Group []string `protobuf:"bytes,1,rep,name=group,proto3" json:"group,omitempty"`
	// 在专属主机或共享池中创建弹性云服务器。默认为在共享池创建。值为： shared或dedicated。  - shared：表示共享池。 - dedicated:表示专属主机。  创建与查询此值均有效。
	Tenancy []string `protobuf:"bytes,2,rep,name=tenancy,proto3" json:"tenancy,omitempty"`
	// 专属主机ID。  此属性仅在tenancy值为dedicated时有效。  不指定此属性，系统将自动分配租户可自动放置弹性云服务器的专属主机。  创建与查询此值均有效。
	DedicatedHostId []string `protobuf:"bytes,3,rep,name=dedicated_host_id,json=dedicatedHostId,proto3" json:"dedicated_host_id,omitempty"`
}

func (x *ListInstancesResponse_Instance_ServerSchedulerHints) Reset() {
	*x = ListInstancesResponse_Instance_ServerSchedulerHints{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[56]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstancesResponse_Instance_ServerSchedulerHints) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstancesResponse_Instance_ServerSchedulerHints) ProtoMessage() {}

func (x *ListInstancesResponse_Instance_ServerSchedulerHints) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[56]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstancesResponse_Instance_ServerSchedulerHints.ProtoReflect.Descriptor instead.
func (*ListInstancesResponse_Instance_ServerSchedulerHints) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{3, 0, 8}
}

func (x *ListInstancesResponse_Instance_ServerSchedulerHints) GetGroup() []string {
	if x != nil {
		return x.Group
	}
	return nil
}

func (x *ListInstancesResponse_Instance_ServerSchedulerHints) GetTenancy() []string {
	if x != nil {
		return x.Tenancy
	}
	return nil
}

func (x *ListInstancesResponse_Instance_ServerSchedulerHints) GetDedicatedHostId() []string {
	if x != nil {
		return x.DedicatedHostId
	}
	return nil
}

// 弹性云服务器系统标签
type ListInstancesResponse_Instance_ServerSystemTag struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 系统标签的Key值。
	Key string `protobuf:"bytes,1,opt,name=key,proto3" json:"key,omitempty"`
	// 系统标签的value值。
	Value string `protobuf:"bytes,2,opt,name=value,proto3" json:"value,omitempty"`
}

func (x *ListInstancesResponse_Instance_ServerSystemTag) Reset() {
	*x = ListInstancesResponse_Instance_ServerSystemTag{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[57]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstancesResponse_Instance_ServerSystemTag) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstancesResponse_Instance_ServerSystemTag) ProtoMessage() {}

func (x *ListInstancesResponse_Instance_ServerSystemTag) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[57]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstancesResponse_Instance_ServerSystemTag.ProtoReflect.Descriptor instead.
func (*ListInstancesResponse_Instance_ServerSystemTag) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{3, 0, 9}
}

func (x *ListInstancesResponse_Instance_ServerSystemTag) GetKey() string {
	if x != nil {
		return x.Key
	}
	return ""
}

func (x *ListInstancesResponse_Instance_ServerSystemTag) GetValue() string {
	if x != nil {
		return x.Value
	}
	return ""
}

type ListInstancesResponse_Instance_CpuOptions struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// CPU超线程数， 决定CPU是否开启超线程
	HwCpuThreads int32 `protobuf:"varint,1,opt,name=hw_cpu_threads,json=hwCpuThreads,proto3" json:"hw_cpu_threads,omitempty"`
}

func (x *ListInstancesResponse_Instance_CpuOptions) Reset() {
	*x = ListInstancesResponse_Instance_CpuOptions{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[58]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstancesResponse_Instance_CpuOptions) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstancesResponse_Instance_CpuOptions) ProtoMessage() {}

func (x *ListInstancesResponse_Instance_CpuOptions) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[58]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstancesResponse_Instance_CpuOptions.ProtoReflect.Descriptor instead.
func (*ListInstancesResponse_Instance_CpuOptions) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{3, 0, 10}
}

func (x *ListInstancesResponse_Instance_CpuOptions) GetHwCpuThreads() int32 {
	if x != nil {
		return x.HwCpuThreads
	}
	return 0
}

type ListInstancesResponse_Instance_Hypervisor struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// hypervisor类型
	HypervisorType string `protobuf:"bytes,1,opt,name=hypervisor_type,json=hypervisorType,proto3" json:"hypervisor_type,omitempty"`
	// hypervisor csd信息
	CsdHypervisor string `protobuf:"bytes,2,opt,name=csd_hypervisor,json=csdHypervisor,proto3" json:"csd_hypervisor,omitempty"`
}

func (x *ListInstancesResponse_Instance_Hypervisor) Reset() {
	*x = ListInstancesResponse_Instance_Hypervisor{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[59]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstancesResponse_Instance_Hypervisor) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstancesResponse_Instance_Hypervisor) ProtoMessage() {}

func (x *ListInstancesResponse_Instance_Hypervisor) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[59]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstancesResponse_Instance_Hypervisor.ProtoReflect.Descriptor instead.
func (*ListInstancesResponse_Instance_Hypervisor) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{3, 0, 11}
}

func (x *ListInstancesResponse_Instance_Hypervisor) GetHypervisorType() string {
	if x != nil {
		return x.HypervisorType
	}
	return ""
}

func (x *ListInstancesResponse_Instance_Hypervisor) GetCsdHypervisor() string {
	if x != nil {
		return x.CsdHypervisor
	}
	return ""
}

type ListRegionsResponse_Region struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	EndPoint string `protobuf:"bytes,1,opt,name=end_point,json=endPoint,proto3" json:"end_point,omitempty"`
	Id       string `protobuf:"bytes,2,opt,name=id,proto3" json:"id,omitempty"`
}

func (x *ListRegionsResponse_Region) Reset() {
	*x = ListRegionsResponse_Region{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[60]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListRegionsResponse_Region) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListRegionsResponse_Region) ProtoMessage() {}

func (x *ListRegionsResponse_Region) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[60]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListRegionsResponse_Region.ProtoReflect.Descriptor instead.
func (*ListRegionsResponse_Region) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{5, 0}
}

func (x *ListRegionsResponse_Region) GetEndPoint() string {
	if x != nil {
		return x.EndPoint
	}
	return ""
}

func (x *ListRegionsResponse_Region) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

type ListImagesResponse_Image struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 镜像ID
	ImageId string `protobuf:"bytes,1,opt,name=image_id,json=imageId,proto3" json:"image_id,omitempty"`
	// 镜像名称
	ImageName string `protobuf:"bytes,2,opt,name=image_name,json=imageName,proto3" json:"image_name,omitempty"`
	// 镜像描述信息
	ImageDescription string `protobuf:"bytes,3,opt,name=image_description,json=imageDescription,proto3" json:"image_description,omitempty"`
	// 镜像文件的大小
	ImageSize int64 `protobuf:"varint,4,opt,name=image_size,json=imageSize,proto3" json:"image_size,omitempty"`
	// 镜像类型，目前支持以下类型：
	// 公共镜像：gold
	// 私有镜像：private
	// 共享镜像：shared
	ImageType string `protobuf:"bytes,5,opt,name=image_type,json=imageType,proto3" json:"image_type,omitempty"`
	// 是否被其他租户可见。取值如下：
	//    public：表示公共镜像
	//    private：表示私有镜像
	//    shared：表示共享镜像
	Visibility string `protobuf:"bytes,6,opt,name=visibility,proto3" json:"visibility,omitempty"`
	// 镜像状态。取值如下：
	//    queued：表示镜像元数据已经创建成功，等待上传镜像文件。
	//    saving：表示镜像正在上传文件到后端存储。
	//    deleted：表示镜像已经删除。
	//    killed：表示镜像上传错误。
	//    active：表示镜像可以正常使用。
	Status string `protobuf:"bytes,7,opt,name=status,proto3" json:"status,omitempty"`
	// 镜像运行需要的最小内存，单位为MB。参数取值依据云服务器的规格限制，一般为0
	MinRam int32 `protobuf:"varint,8,opt,name=min_ram,json=minRam,proto3" json:"min_ram,omitempty"`
	// 镜像支持的最大内存，单位为MB。
	MaxRam string `protobuf:"bytes,9,opt,name=max_ram,json=maxRam,proto3" json:"max_ram,omitempty"`
	// 镜像运行需要的最小磁盘容量，单位为GB 。取值为40～1024GB。
	MinDisk int32 `protobuf:"varint,10,opt,name=min_disk,json=minDisk,proto3" json:"min_disk,omitempty"`
	// 操作系统位数，一般取值为32或者64。
	OsBit string `protobuf:"bytes,11,opt,name=os_bit,json=osBit,proto3" json:"os_bit,omitempty"`
	// 镜像平台分类，取值为Windows、Ubuntu、RedHat、SUSE、CentOS、Debian、OpenSUSE、Oracle Linux、Fedora、Other、CoreOS和EulerOS
	Platform string `protobuf:"bytes,12,opt,name=platform,proto3" json:"platform,omitempty"`
	// 镜像系统类型，取值为Linux、Windows、Other
	OsType string `protobuf:"bytes,13,opt,name=os_type,json=osType,proto3" json:"os_type,omitempty"`
	// 操作系统具体版本
	OsVersion string `protobuf:"bytes,14,opt,name=os_version,json=osVersion,proto3" json:"os_version,omitempty"`
	// 创建时间。格式为UTC时间。
	CreatedAt string `protobuf:"bytes,15,opt,name=created_at,json=createdAt,proto3" json:"created_at,omitempty"`
	// 更新时间。格式为UTC时间。
	UpdatedAt string `protobuf:"bytes,16,opt,name=updated_at,json=updatedAt,proto3" json:"updated_at,omitempty"`
	// 镜像状态变为正常的时间。
	ActiveAt string `protobuf:"bytes,17,opt,name=active_at,json=activeAt,proto3" json:"active_at,omitempty"`
}

func (x *ListImagesResponse_Image) Reset() {
	*x = ListImagesResponse_Image{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[61]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListImagesResponse_Image) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListImagesResponse_Image) ProtoMessage() {}

func (x *ListImagesResponse_Image) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[61]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListImagesResponse_Image.ProtoReflect.Descriptor instead.
func (*ListImagesResponse_Image) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{7, 0}
}

func (x *ListImagesResponse_Image) GetImageId() string {
	if x != nil {
		return x.ImageId
	}
	return ""
}

func (x *ListImagesResponse_Image) GetImageName() string {
	if x != nil {
		return x.ImageName
	}
	return ""
}

func (x *ListImagesResponse_Image) GetImageDescription() string {
	if x != nil {
		return x.ImageDescription
	}
	return ""
}

func (x *ListImagesResponse_Image) GetImageSize() int64 {
	if x != nil {
		return x.ImageSize
	}
	return 0
}

func (x *ListImagesResponse_Image) GetImageType() string {
	if x != nil {
		return x.ImageType
	}
	return ""
}

func (x *ListImagesResponse_Image) GetVisibility() string {
	if x != nil {
		return x.Visibility
	}
	return ""
}

func (x *ListImagesResponse_Image) GetStatus() string {
	if x != nil {
		return x.Status
	}
	return ""
}

func (x *ListImagesResponse_Image) GetMinRam() int32 {
	if x != nil {
		return x.MinRam
	}
	return 0
}

func (x *ListImagesResponse_Image) GetMaxRam() string {
	if x != nil {
		return x.MaxRam
	}
	return ""
}

func (x *ListImagesResponse_Image) GetMinDisk() int32 {
	if x != nil {
		return x.MinDisk
	}
	return 0
}

func (x *ListImagesResponse_Image) GetOsBit() string {
	if x != nil {
		return x.OsBit
	}
	return ""
}

func (x *ListImagesResponse_Image) GetPlatform() string {
	if x != nil {
		return x.Platform
	}
	return ""
}

func (x *ListImagesResponse_Image) GetOsType() string {
	if x != nil {
		return x.OsType
	}
	return ""
}

func (x *ListImagesResponse_Image) GetOsVersion() string {
	if x != nil {
		return x.OsVersion
	}
	return ""
}

func (x *ListImagesResponse_Image) GetCreatedAt() string {
	if x != nil {
		return x.CreatedAt
	}
	return ""
}

func (x *ListImagesResponse_Image) GetUpdatedAt() string {
	if x != nil {
		return x.UpdatedAt
	}
	return ""
}

func (x *ListImagesResponse_Image) GetActiveAt() string {
	if x != nil {
		return x.ActiveAt
	}
	return ""
}

type ListInstanceTypesResponse_InstanceType struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 云服务器规格的ID
	Id string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	// 云服务器规格的名称
	Name string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// 云服务器规格对应的CPU核数
	VCpus string `protobuf:"bytes,3,opt,name=v_cpus,json=vCpus,proto3" json:"v_cpus,omitempty"`
	// 云服务器规格对应的内存大小，单位为MB。
	MemorySize               int32                                                            `protobuf:"varint,4,opt,name=memory_size,json=memorySize,proto3" json:"memory_size,omitempty"`
	OsExtraSpec              *ListInstanceTypesResponse_InstanceType_OsExtraSpec              `protobuf:"bytes,5,opt,name=os_extra_spec,json=osExtraSpec,proto3" json:"os_extra_spec,omitempty"`
	ServerAttachableQuantity *ListInstanceTypesResponse_InstanceType_ServerAttachableQuantity `protobuf:"bytes,6,opt,name=server_attachable_quantity,json=serverAttachableQuantity,proto3" json:"server_attachable_quantity,omitempty"`
}

func (x *ListInstanceTypesResponse_InstanceType) Reset() {
	*x = ListInstanceTypesResponse_InstanceType{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[62]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstanceTypesResponse_InstanceType) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstanceTypesResponse_InstanceType) ProtoMessage() {}

func (x *ListInstanceTypesResponse_InstanceType) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[62]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstanceTypesResponse_InstanceType.ProtoReflect.Descriptor instead.
func (*ListInstanceTypesResponse_InstanceType) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{9, 0}
}

func (x *ListInstanceTypesResponse_InstanceType) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType) GetVCpus() string {
	if x != nil {
		return x.VCpus
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType) GetMemorySize() int32 {
	if x != nil {
		return x.MemorySize
	}
	return 0
}

func (x *ListInstanceTypesResponse_InstanceType) GetOsExtraSpec() *ListInstanceTypesResponse_InstanceType_OsExtraSpec {
	if x != nil {
		return x.OsExtraSpec
	}
	return nil
}

func (x *ListInstanceTypesResponse_InstanceType) GetServerAttachableQuantity() *ListInstanceTypesResponse_InstanceType_ServerAttachableQuantity {
	if x != nil {
		return x.ServerAttachableQuantity
	}
	return nil
}

type ListInstanceTypesResponse_InstanceType_OsExtraSpec struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 云服务器规格的分类：  - normal：通用型 - cpuv1：计算I型 - cpuv2：计算II型 - highmem：内存优化型 - gpu：GPU加速型 - entry：通用入门型 - saphana：大内存型 - ultracpu：超高性能计算型 - diskintensive：磁盘增强型 - highio：超高I/O型 - fpga：FPGA加速型  > 说明：  - 早期注册的规格该字段为hws:performancetype。
	EcsPerformanceType string `protobuf:"bytes,1,opt,name=ecs_performance_type,json=ecsPerformanceType,proto3" json:"ecs_performance_type,omitempty"`
	// 主机的物理cpu数量。
	HwNumaNodes string `protobuf:"bytes,2,opt,name=hw_numa_nodes,json=hwNumaNodes,proto3" json:"hw_numa_nodes,omitempty"`
	// 资源类型。resource_type是为了区分云服务器的物理主机类型。
	ResourceType string `protobuf:"bytes,3,opt,name=resource_type,json=resourceType,proto3" json:"resource_type,omitempty"`
	// 弹性运服务器高精度时钟是否开启，开启为true，否则为false。（该字段是否返回根据云服务器规格而定）
	HPetSupport string `protobuf:"bytes,4,opt,name=h_pet_support,json=hPetSupport,proto3" json:"h_pet_support,omitempty"`
	// 网卡类型，值固定为“enhanced”，表示使用增强型网络的资源创建云服务器。
	InstanceVNicType string `protobuf:"bytes,5,opt,name=instance_v_nic_type,json=instanceVNicType,proto3" json:"instance_v_nic_type,omitempty"`
	// 最大带宽，单位Mbps，最大值为10000。
	InstanceVNicInstanceBandwidth string `protobuf:"bytes,6,opt,name=instance_v_nic_instance_bandwidth,json=instanceVNicInstanceBandwidth,proto3" json:"instance_v_nic_instance_bandwidth,omitempty"`
	// 最大网卡个数，最大为4。
	InstanceVnicMaxCount string `protobuf:"bytes,7,opt,name=instance_vnic_max_count,json=instanceVnicMaxCount,proto3" json:"instance_vnic_max_count,omitempty"`
	// 值格式为{type}:{count}:{size}:{safeFormat}，其中：  - type指磁盘类型，当前只支持hdd。 - count指本地磁盘数量，目前支持d1类型：3/6/12/24，d2类型：2/4/8/12/16/24，d3类型：2/4/8/12/16/24/28。 - size指单个磁盘容量，单位GB，目前只支持1675（实际磁盘大小为1800，格式化后可用大小为1675）。 - safeFormat指云服务器本地磁盘是否安全格式化，目前仅支持d1类型：FALSE，d2/d3类型：True。  > 说明：  - 磁盘增强型特有字段。
	QuotaLocalDisk string `protobuf:"bytes,8,opt,name=quota_local_disk,json=quotaLocalDisk,proto3" json:"quota_local_disk,omitempty"`
	// 值格式为{type}:{spec}:{size}:{safeFormat}，其中：  - type指主机上配备的nvme ssd的单卡容量大小，当前只支持1.6T/3.2T。 - spec指nvme ssd的规格，包括large/small。large表示大规格，small表示小规格。目前仅支持i3类型：large。 - size指guest使用的盘的容量大小，单位为GB。在spec值为large的情况下，此项即为host单卡大小。在spec值为small的情况下，此为1/4规格或者1/2规格。 - safeFormat指云服务器本地磁盘是否安全格式化，目前仅支持i3类型：True。  > 说明：  - 超高I/O型特有字段。
	QuotaNvmeSsd string `protobuf:"bytes,9,opt,name=quota_nvme_ssd,json=quotaNvmeSsd,proto3" json:"quota_nvme_ssd,omitempty"`
	// 是否支持持久化，值为true。  代表云服务器访问存储的方式为持久化授权。   > 说明：  - 密集存储D1型特有字段。
	ExtraSpecIoPersistentGrant string `protobuf:"bytes,10,opt,name=extra_spec_io_persistent_grant,json=extraSpecIoPersistentGrant,proto3" json:"extra_spec_io_persistent_grant,omitempty"`
	// 弹性云服务器类型的代数。  - s1：通用型I代 - s2：通用型II代 - s3：通用型 - m1：内存优化型I代 - m2：内存优化型II代 - m3：内存优化型 - h1：高性能计算型I代 - h2：高性能计算型II代 - h3：高性能计算型 - hi3：超高性能计算型 - d1：密集存储型I代 - d2：密集存储型II代 - d3：磁盘增强型 - g1：GPU加速型I代 - g2：GPU加速型II代 - f1：FPGA高性能型 - f2：FPGA通用型 - c3：通用计算增强型 - e3：大内存型 - i3：超高I/O型
	EcsGeneration string `protobuf:"bytes,11,opt,name=ecs_generation,json=ecsGeneration,proto3" json:"ecs_generation,omitempty"`
	// 虚拟化类型。  - 如果值为“FusionCompute”，表示弹性云服务器使用基于XEN的虚拟化技术。 - 如果值为“CloudCompute”，表示弹性云服务器使用基于KVM的虚拟化技术。
	EcsVirtualizationEnvTypes string `protobuf:"bytes,12,opt,name=ecs_virtualization_env_types,json=ecsVirtualizationEnvTypes,proto3" json:"ecs_virtualization_env_types,omitempty"`
	// 显卡是否直通。  值为“true”，表示GPU直通。
	PciPassthroughEnableGpu string `protobuf:"bytes,13,opt,name=pci_passthrough_enable_gpu,json=pciPassthroughEnableGpu,proto3" json:"pci_passthrough_enable_gpu,omitempty"`
	// G1型和G2型云服务器应用的技术，包括GPU虚拟化和GPU直通。  - 如果该规格的云服务器使用GPU虚拟化技术，且GPU卡的型号为M60-1Q，参数值可设置为“m60_1q:virt:1”。 - 如果该规格的云服务器使用GPU直通技术，且GPU卡的型号为M60，参数值可设置为“m60:direct_graphics:1”。
	PciPassthroughGpuSpecs string `protobuf:"bytes,14,opt,name=pci_passthrough_gpu_specs,json=pciPassthroughGpuSpecs,proto3" json:"pci_passthrough_gpu_specs,omitempty"`
	// P1型v本地直通GPU的型号和数量，参数值可设置为“nvidia-p100:1”，表示使用该规格创建的弹性云服务器将占用1张NVIDIA P100显卡。
	PciPassthroughAlias string `protobuf:"bytes,15,opt,name=pci_passthrough_alias,json=pciPassthroughAlias,proto3" json:"pci_passthrough_alias,omitempty"`
	// 此参数是Region级配置，某个AZ没有在cond:operation:az参数中配置时默认使用此参数的取值。不配置或无此参数时等同于“normal”。取值范围：  - normal：正常商用 - abandon：下线（即不显示） - sellout：售罄 - obt：公测 - promotion：推荐(等同normal，也是商用)
	CondOperationStatus string `protobuf:"bytes,16,opt,name=cond_operation_status,json=condOperationStatus,proto3" json:"cond_operation_status,omitempty"`
	// 此参数是AZ级配置，某个AZ没有在此参数中配置时默认使用cond:operation:status参数的取值。此参数的配置格式“az(xx)”。()内为某个AZ的flavor状态，()内必须要填有状态，不填为无效配置。状态的取值范围与cond:operation:status参数相同。  例如：flavor在某个region的az0正常商用，az1售罄，az2公测，az3正常商用，其他az显示下线，可配置为：  - “cond:operation:status”设置为“abandon” - “cond:operation:az”设置为“az0(normal), az1(sellout), az2(obt), az3(normal)”  > 说明：  - 如果flavor在某个AZ下的状态与cond:operation:status配置状态不同，必须配置该参数。
	CondOperationAz string `protobuf:"bytes,17,opt,name=cond_operation_az,json=condOperationAz,proto3" json:"cond_operation_az,omitempty"`
	// 最大带宽  - 单位Mbps，显示为Gbps时除以1000
	QuotaMaxRate string `protobuf:"bytes,18,opt,name=quota_max_rate,json=quotaMaxRate,proto3" json:"quota_max_rate,omitempty"`
	// 基准带宽  - 单位Mbps，显示为Gbps时除以1000
	QuotaMinRate string `protobuf:"bytes,19,opt,name=quota_min_rate,json=quotaMinRate,proto3" json:"quota_min_rate,omitempty"`
	// 内网最大收发包能力  - 单位个，显示为xx万时除以10000
	QuotaMaxPps string `protobuf:"bytes,20,opt,name=quota_max_pps,json=quotaMaxPps,proto3" json:"quota_max_pps,omitempty"`
	// 计费类型  - 计费场景，不配置时都支持 - period，包周期 - demand，按需
	CondOperationCharge string `protobuf:"bytes,21,opt,name=cond_operation_charge,json=condOperationCharge,proto3" json:"cond_operation_charge,omitempty"`
	// 关机是否收费  - 关机是否计费，默认免费： - charge - free
	CondOperationChargeStop string `protobuf:"bytes,22,opt,name=cond_operation_charge_stop,json=condOperationChargeStop,proto3" json:"cond_operation_charge_stop,omitempty"`
	// 计费类型  - 计费场景，不配置时都支持 - period，包周期 - demand，按需
	CondSpotOperationAz string `protobuf:"bytes,23,opt,name=cond_spot_operation_az,json=condSpotOperationAz,proto3" json:"cond_spot_operation_az,omitempty"`
	// 允许的角色 匹配的用户标签（roles的op_gatexxx标签）。不设置时所有用户可见
	CondOperationRoles string `protobuf:"bytes,24,opt,name=cond_operation_roles,json=condOperationRoles,proto3" json:"cond_operation_roles,omitempty"`
	// Flavor在竞价销售模式下的状态  - 不配置时等同abandon - normal，正常商用 - abandon，下线 - sellout，售罄 - obt，公测，未申请时提示申请（暂不支持） - private，私有，只给特定用户显示（暂不支持） - test，试用/免费（暂不支持） - promotion，推荐
	CondSpotOperationStatus string `protobuf:"bytes,25,opt,name=cond_spot_operation_status,json=condSpotOperationStatus,proto3" json:"cond_spot_operation_status,omitempty"`
	// 网络约束 支持网络特性，不配置时以UI配置为准。
	CondNetwork string `protobuf:"bytes,26,opt,name=cond_network,json=condNetwork,proto3" json:"cond_network,omitempty"`
	// 存储约束  - 支持磁盘特性，不配置时以UI配置为准。 - scsi，支持scsi - localdisk，支持本地盘 - ib，支持ib
	CondStorage string `protobuf:"bytes,27,opt,name=cond_storage,json=condStorage,proto3" json:"cond_storage,omitempty"`
	// 计算约束  - true，支持在线扩容。 - false或不存在该字段，不支持在线扩容。
	CondComputeLiveResizable string `protobuf:"bytes,28,opt,name=cond_compute_live_resizable,json=condComputeLiveResizable,proto3" json:"cond_compute_live_resizable,omitempty"`
	// 计算约束  - autorecovery，自动恢复特性。 - 不存在该字段，不支持自动恢复。
	CondCompute string `protobuf:"bytes,29,opt,name=cond_compute,json=condCompute,proto3" json:"cond_compute,omitempty"`
	// GPU显卡数量和名称。
	InfoGpuName string `protobuf:"bytes,30,opt,name=info_gpu_name,json=infoGpuName,proto3" json:"info_gpu_name,omitempty"`
	// CPU名称
	InfoCpuName string `protobuf:"bytes,31,opt,name=info_cpu_name,json=infoCpuName,proto3" json:"info_cpu_name,omitempty"`
	// GPU显卡名称。
	QuotaGpu string `protobuf:"bytes,32,opt,name=quota_gpu,json=quotaGpu,proto3" json:"quota_gpu,omitempty"`
	// 该规格对应的CPU架构，且仅鲲鹏实例架构规格返回该字段  - 取值为arm64表示CPU架构为鲲鹏计算。
	EcsInstanceArchitecture string `protobuf:"bytes,33,opt,name=ecs_instance_architecture,json=ecsInstanceArchitecture,proto3" json:"ecs_instance_architecture,omitempty"`
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) Reset() {
	*x = ListInstanceTypesResponse_InstanceType_OsExtraSpec{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[63]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstanceTypesResponse_InstanceType_OsExtraSpec) ProtoMessage() {}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[63]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstanceTypesResponse_InstanceType_OsExtraSpec.ProtoReflect.Descriptor instead.
func (*ListInstanceTypesResponse_InstanceType_OsExtraSpec) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{9, 0, 0}
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetEcsPerformanceType() string {
	if x != nil {
		return x.EcsPerformanceType
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetHwNumaNodes() string {
	if x != nil {
		return x.HwNumaNodes
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetResourceType() string {
	if x != nil {
		return x.ResourceType
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetHPetSupport() string {
	if x != nil {
		return x.HPetSupport
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetInstanceVNicType() string {
	if x != nil {
		return x.InstanceVNicType
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetInstanceVNicInstanceBandwidth() string {
	if x != nil {
		return x.InstanceVNicInstanceBandwidth
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetInstanceVnicMaxCount() string {
	if x != nil {
		return x.InstanceVnicMaxCount
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetQuotaLocalDisk() string {
	if x != nil {
		return x.QuotaLocalDisk
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetQuotaNvmeSsd() string {
	if x != nil {
		return x.QuotaNvmeSsd
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetExtraSpecIoPersistentGrant() string {
	if x != nil {
		return x.ExtraSpecIoPersistentGrant
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetEcsGeneration() string {
	if x != nil {
		return x.EcsGeneration
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetEcsVirtualizationEnvTypes() string {
	if x != nil {
		return x.EcsVirtualizationEnvTypes
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetPciPassthroughEnableGpu() string {
	if x != nil {
		return x.PciPassthroughEnableGpu
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetPciPassthroughGpuSpecs() string {
	if x != nil {
		return x.PciPassthroughGpuSpecs
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetPciPassthroughAlias() string {
	if x != nil {
		return x.PciPassthroughAlias
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetCondOperationStatus() string {
	if x != nil {
		return x.CondOperationStatus
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetCondOperationAz() string {
	if x != nil {
		return x.CondOperationAz
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetQuotaMaxRate() string {
	if x != nil {
		return x.QuotaMaxRate
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetQuotaMinRate() string {
	if x != nil {
		return x.QuotaMinRate
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetQuotaMaxPps() string {
	if x != nil {
		return x.QuotaMaxPps
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetCondOperationCharge() string {
	if x != nil {
		return x.CondOperationCharge
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetCondOperationChargeStop() string {
	if x != nil {
		return x.CondOperationChargeStop
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetCondSpotOperationAz() string {
	if x != nil {
		return x.CondSpotOperationAz
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetCondOperationRoles() string {
	if x != nil {
		return x.CondOperationRoles
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetCondSpotOperationStatus() string {
	if x != nil {
		return x.CondSpotOperationStatus
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetCondNetwork() string {
	if x != nil {
		return x.CondNetwork
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetCondStorage() string {
	if x != nil {
		return x.CondStorage
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetCondComputeLiveResizable() string {
	if x != nil {
		return x.CondComputeLiveResizable
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetCondCompute() string {
	if x != nil {
		return x.CondCompute
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetInfoGpuName() string {
	if x != nil {
		return x.InfoGpuName
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetInfoCpuName() string {
	if x != nil {
		return x.InfoCpuName
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetQuotaGpu() string {
	if x != nil {
		return x.QuotaGpu
	}
	return ""
}

func (x *ListInstanceTypesResponse_InstanceType_OsExtraSpec) GetEcsInstanceArchitecture() string {
	if x != nil {
		return x.EcsInstanceArchitecture
	}
	return ""
}

type ListInstanceTypesResponse_InstanceType_ServerAttachableQuantity struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	// 可挂载scsi卷数。
	FreeScsi int32 `protobuf:"varint,1,opt,name=free_scsi,json=freeScsi,proto3" json:"free_scsi,omitempty"`
	// 可挂载vbd卷数。
	FreeBlk int32 `protobuf:"varint,2,opt,name=free_blk,json=freeBlk,proto3" json:"free_blk,omitempty"`
	// 可挂载卷数。
	FreeDisk int32 `protobuf:"varint,3,opt,name=free_disk,json=freeDisk,proto3" json:"free_disk,omitempty"`
	// 可挂载网卡数。
	FreeNic int32 `protobuf:"varint,4,opt,name=free_nic,json=freeNic,proto3" json:"free_nic,omitempty"`
}

func (x *ListInstanceTypesResponse_InstanceType_ServerAttachableQuantity) Reset() {
	*x = ListInstanceTypesResponse_InstanceType_ServerAttachableQuantity{}
	if protoimpl.UnsafeEnabled {
		mi := &file_api_instance_v1_instance_proto_msgTypes[64]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *ListInstanceTypesResponse_InstanceType_ServerAttachableQuantity) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*ListInstanceTypesResponse_InstanceType_ServerAttachableQuantity) ProtoMessage() {}

func (x *ListInstanceTypesResponse_InstanceType_ServerAttachableQuantity) ProtoReflect() protoreflect.Message {
	mi := &file_api_instance_v1_instance_proto_msgTypes[64]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use ListInstanceTypesResponse_InstanceType_ServerAttachableQuantity.ProtoReflect.Descriptor instead.
func (*ListInstanceTypesResponse_InstanceType_ServerAttachableQuantity) Descriptor() ([]byte, []int) {
	return file_api_instance_v1_instance_proto_rawDescGZIP(), []int{9, 0, 1}
}

func (x *ListInstanceTypesResponse_InstanceType_ServerAttachableQuantity) GetFreeScsi() int32 {
	if x != nil {
		return x.FreeScsi
	}
	return 0
}

func (x *ListInstanceTypesResponse_InstanceType_ServerAttachableQuantity) GetFreeBlk() int32 {
	if x != nil {
		return x.FreeBlk
	}
	return 0
}

func (x *ListInstanceTypesResponse_InstanceType_ServerAttachableQuantity) GetFreeDisk() int32 {
	if x != nil {
		return x.FreeDisk
	}
	return 0
}

func (x *ListInstanceTypesResponse_InstanceType_ServerAttachableQuantity) GetFreeNic() int32 {
	if x != nil {
		return x.FreeNic
	}
	return 0
}

var File_api_instance_v1_instance_proto protoreflect.FileDescriptor

var file_api_instance_v1_instance_proto_rawDesc = []byte{
	,
}

var (
	file_api_instance_v1_instance_proto_rawDescOnce sync.Once
	file_api_instance_v1_instance_proto_rawDescData = file_api_instance_v1_instance_proto_rawDesc
)

func file_api_instance_v1_instance_proto_rawDescGZIP() []byte {
	file_api_instance_v1_instance_proto_rawDescOnce.Do(func() {
		file_api_instance_v1_instance_proto_rawDescData = protoimpl.X.CompressGZIP(file_api_instance_v1_instance_proto_rawDescData)
	})
	return file_api_instance_v1_instance_proto_rawDescData
}

var file_api_instance_v1_instance_proto_msgTypes = make([]protoimpl.MessageInfo, 65)
var file_api_instance_v1_instance_proto_goTypes = []interface{}{
	(*CreateInstancesRequest)(nil),                         // 0: api.instance.v1.CreateInstancesRequest
	(*CreateInstancesResponse)(nil),                        // 1: api.instance.v1.CreateInstancesResponse
	(*ListInstancesRequest)(nil),                           // 2: api.instance.v1.ListInstancesRequest
	(*ListInstancesResponse)(nil),                          // 3: api.instance.v1.ListInstancesResponse
	(*ListRegionsRequest)(nil),                             // 4: api.instance.v1.ListRegionsRequest
	(*ListRegionsResponse)(nil),                            // 5: api.instance.v1.ListRegionsResponse
	(*ListImagesRequest)(nil),                              // 6: api.instance.v1.ListImagesRequest
	(*ListImagesResponse)(nil),                             // 7: api.instance.v1.ListImagesResponse
	(*ListInstanceTypesRequest)(nil),                       // 8: api.instance.v1.ListInstanceTypesRequest
	(*ListInstanceTypesResponse)(nil),                      // 9: api.instance.v1.ListInstanceTypesResponse
	(*StartInstancesRequest)(nil),                          // 10: api.instance.v1.StartInstancesRequest
	(*StartInstancesResponse)(nil),                         // 11: api.instance.v1.StartInstancesResponse
	(*StopInstancesRequest)(nil),                           // 12: api.instance.v1.StopInstancesRequest
	(*StopInstancesResponse)(nil),                          // 13: api.instance.v1.StopInstancesResponse
	(*RebootInstancesRequest)(nil),                         // 14: api.instance.v1.RebootInstancesRequest
	(*RebootInstancesResponse)(nil),                        // 15: api.instance.v1.RebootInstancesResponse
	(*ServerId)(nil),                                       // 16: api.instance.v1.ServerId
	(*DeleteInstancesRequest)(nil),                         // 17: api.instance.v1.DeleteInstancesRequest
	(*DeleteInstancesResponse)(nil),                        // 18: api.instance.v1.DeleteInstancesResponse
	(*Vpc)(nil),                                            // 19: api.instance.v1.Vpc
	(*VpcRoute)(nil),                                       // 20: api.instance.v1.VpcRoute
	(*CreateVpcRequest)(nil),                               // 21: api.instance.v1.CreateVpcRequest
	(*CreateVpcResponse)(nil),                              // 22: api.instance.v1.CreateVpcResponse
	(*ListVpcsRequest)(nil),                                // 23: api.instance.v1.ListVpcsRequest
	(*ListVpcsResponse)(nil),                               // 24: api.instance.v1.ListVpcsResponse
	(*UpdateVpcRequest)(nil),                               // 25: api.instance.v1.UpdateVpcRequest
	(*UpdateVpcResponse)(nil),                              // 26: api.instance.v1.UpdateVpcResponse
	(*DeleteVpcRequest)(nil),                               // 27: api.instance.v1.DeleteVpcRequest
	(*DeleteVpcResponse)(nil),                              // 28: api.instance.v1.DeleteVpcResponse
	(*ShowVpcRequest)(nil),                                 // 29: api.instance.v1.ShowVpcRequest
	(*ShowVpcResponse)(nil),                                // 30: api.instance.v1.ShowVpcResponse
	(*Subnet)(nil),                                         // 31: api.instance.v1.Subnet
	(*ExtraDhcpOpts)(nil),                                  // 32: api.instance.v1.ExtraDhcpOpts
	(*CreateSubnetRequest)(nil),                            // 33: api.instance.v1.CreateSubnetRequest
	(*CreateSubnetResponse)(nil),                           // 34: api.instance.v1.CreateSubnetResponse
	(*ListSubnetsRequest)(nil),                             // 35: api.instance.v1.ListSubnetsRequest
	(*ListSubnetsResponse)(nil),                            // 36: api.instance.v1.ListSubnetsResponse
	(*UpdateSubnetRequest)(nil),                            // 37: api.instance.v1.UpdateSubnetRequest
	(*UpdateSubnetResponse)(nil),                           // 38: api.instance.v1.UpdateSubnetResponse
	(*DeleteSubnetRequest)(nil),                            // 39: api.instance.v1.DeleteSubnetRequest
	(*DeleteSubnetResponse)(nil),                           // 40: api.instance.v1.DeleteSubnetResponse
	(*ShowSubnetRequest)(nil),                              // 41: api.instance.v1.ShowSubnetRequest
	(*ShowSubnetResponse)(nil),                             // 42: api.instance.v1.ShowSubnetResponse
	(*ShowJobRequest)(nil),                                 // 43: api.instance.v1.ShowJobRequest
	(*ShowJobResponse)(nil),                                // 44: api.instance.v1.ShowJobResponse
	(*CreateInstancesRequest_Nic)(nil),                     // 45: api.instance.v1.CreateInstancesRequest.Nic
	(*CreateInstancesRequest_RootVolume)(nil),              // 46: api.instance.v1.CreateInstancesRequest.RootVolume
	(*ListInstancesResponse_Instance)(nil),                 // 47: api.instance.v1.ListInstancesResponse.Instance
	(*ListInstancesResponse_Instance_ServerAddress)(nil),   // 48: api.instance.v1.ListInstancesResponse.Instance.ServerAddress
	(*ListInstancesResponse_Instance_ServerAddresses)(nil), // 49: api.instance.v1.ListInstancesResponse.Instance.ServerAddresses
	nil, // 50: api.instance.v1.ListInstancesResponse.Instance.AddressesEntry
	(*ListInstancesResponse_Instance_InstanceType)(nil),  // 51: api.instance.v1.ListInstancesResponse.Instance.InstanceType
	(*ListInstancesResponse_Instance_SecurityGroup)(nil), // 52: api.instance.v1.ListInstancesResponse.Instance.SecurityGroup
	(*ListInstancesResponse_Instance_ServerFault)(nil),   // 53: api.instance.v1.ListInstancesResponse.Instance.ServerFault
	nil, // 54: api.instance.v1.ListInstancesResponse.Instance.MetadataEntry
	(*ListInstancesResponse_Instance_ServerExtendVolumeAttachment)(nil),     // 55: api.instance.v1.ListInstancesResponse.Instance.ServerExtendVolumeAttachment
	(*ListInstancesResponse_Instance_ServerSchedulerHints)(nil),             // 56: api.instance.v1.ListInstancesResponse.Instance.ServerSchedulerHints
	(*ListInstancesResponse_Instance_ServerSystemTag)(nil),                  // 57: api.instance.v1.ListInstancesResponse.Instance.ServerSystemTag
	(*ListInstancesResponse_Instance_CpuOptions)(nil),                       // 58: api.instance.v1.ListInstancesResponse.Instance.CpuOptions
	(*ListInstancesResponse_Instance_Hypervisor)(nil),                       // 59: api.instance.v1.ListInstancesResponse.Instance.Hypervisor
	(*ListRegionsResponse_Region)(nil),                                      // 60: api.instance.v1.ListRegionsResponse.Region
	(*ListImagesResponse_Image)(nil),                                        // 61: api.instance.v1.ListImagesResponse.Image
	(*ListInstanceTypesResponse_InstanceType)(nil),                          // 62: api.instance.v1.ListInstanceTypesResponse.InstanceType
	(*ListInstanceTypesResponse_InstanceType_OsExtraSpec)(nil),              // 63: api.instance.v1.ListInstanceTypesResponse.InstanceType.OsExtraSpec
	(*ListInstanceTypesResponse_InstanceType_ServerAttachableQuantity)(nil), // 64: api.instance.v1.ListInstanceTypesResponse.InstanceType.ServerAttachableQuantity
}
var file_api_instance_v1_instance_proto_depIdxs = []int32{
	45, // 0: api.instance.v1.CreateInstancesRequest.nics:type_name -> api.instance.v1.CreateInstancesRequest.Nic
	46, // 1: api.instance.v1.CreateInstancesRequest.root_volume:type_name -> api.instance.v1.CreateInstancesRequest.RootVolume
	47, // 2: api.instance.v1.ListInstancesResponse.instances:type_name -> api.instance.v1.ListInstancesResponse.Instance
	60, // 3: api.instance.v1.ListRegionsResponse.regions:type_name -> api.instance.v1.ListRegionsResponse.Region
	61, // 4: api.instance.v1.ListImagesResponse.images:type_name -> api.instance.v1.ListImagesResponse.Image
	62, // 5: api.instance.v1.ListInstanceTypesResponse.instance_types:type_name -> api.instance.v1.ListInstanceTypesResponse.InstanceType
	16, // 6: api.instance.v1.StartInstancesRequest.servers:type_name -> api.instance.v1.ServerId
	16, // 7: api.instance.v1.StopInstancesRequest.servers:type_name -> api.instance.v1.ServerId
	16, // 8: api.instance.v1.RebootInstancesRequest.servers:type_name -> api.instance.v1.ServerId
	16, // 9: api.instance.v1.DeleteInstancesRequest.servers:type_name -> api.instance.v1.ServerId
	20, // 10: api.instance.v1.Vpc.routes:type_name -> api.instance.v1.VpcRoute
	19, // 11: api.instance.v1.CreateVpcResponse.vpc:type_name -> api.instance.v1.Vpc
	19, // 12: api.instance.v1.ListVpcsResponse.vpcs:type_name -> api.instance.v1.Vpc
	20, // 13: api.instance.v1.UpdateVpcRequest.routes:type_name -> api.instance.v1.VpcRoute
	19, // 14: api.instance.v1.UpdateVpcResponse.vpc:type_name -> api.instance.v1.Vpc
	19, // 15: api.instance.v1.ShowVpcResponse.vpc:type_name -> api.instance.v1.Vpc
	32, // 16: api.instance.v1.Subnet.extra_dhcp_opts:type_name -> api.instance.v1.ExtraDhcpOpts
	32, // 17: api.instance.v1.CreateSubnetRequest.extra_dhcp_opts:type_name -> api.instance.v1.ExtraDhcpOpts
	31, // 18: api.instance.v1.CreateSubnetResponse.subnet:type_name -> api.instance.v1.Subnet
	31, // 19: api.instance.v1.ListSubnetsResponse.subnets:type_name -> api.instance.v1.Subnet
	32, // 20: api.instance.v1.UpdateSubnetRequest.extra_dhcp_opts:type_name -> api.instance.v1.ExtraDhcpOpts
	31, // 21: api.instance.v1.ShowSubnetResponse.subnet:type_name -> api.instance.v1.Subnet
	50, // 22: api.instance.v1.ListInstancesResponse.Instance.addresses:type_name -> api.instance.v1.ListInstancesResponse.Instance.AddressesEntry
	51, // 23: api.instance.v1.ListInstancesResponse.Instance.instance_type:type_name -> api.instance.v1.ListInstancesResponse.Instance.InstanceType
	52, // 24: api.instance.v1.ListInstancesResponse.Instance.security_groups:type_name -> api.instance.v1.ListInstancesResponse.Instance.SecurityGroup
	53, // 25: api.instance.v1.ListInstancesResponse.Instance.fault:type_name -> api.instance.v1.ListInstancesResponse.Instance.ServerFault
	54, // 26: api.instance.v1.ListInstancesResponse.Instance.metadata:type_name -> api.instance.v1.ListInstancesResponse.Instance.MetadataEntry
	55, // 27: api.instance.v1.ListInstancesResponse.Instance.os_extended_volumes_volumes_Attached:type_name -> api.instance.v1.ListInstancesResponse.Instance.ServerExtendVolumeAttachment
	56, // 28: api.instance.v1.ListInstancesResponse.Instance.os_scheduler_hints:type_name -> api.instance.v1.ListInstancesResponse.Instance.ServerSchedulerHints
	57, // 29: api.instance.v1.ListInstancesResponse.Instance.sys_tags:type_name -> api.instance.v1.ListInstancesResponse.Instance.ServerSystemTag
	58, // 30: api.instance.v1.ListInstancesResponse.Instance.cpu_options:type_name -> api.instance.v1.ListInstancesResponse.Instance.CpuOptions
	59, // 31: api.instance.v1.ListInstancesResponse.Instance.hypervisor:type_name -> api.instance.v1.ListInstancesResponse.Instance.Hypervisor
	48, // 32: api.instance.v1.ListInstancesResponse.Instance.ServerAddresses.server_addresses:type_name -> api.instance.v1.ListInstancesResponse.Instance.ServerAddress
	49, // 33: api.instance.v1.ListInstancesResponse.Instance.AddressesEntry.value:type_name -> api.instance.v1.ListInstancesResponse.Instance.ServerAddresses
	63, // 34: api.instance.v1.ListInstanceTypesResponse.InstanceType.os_extra_spec:type_name -> api.instance.v1.ListInstanceTypesResponse.InstanceType.OsExtraSpec
	64, // 35: api.instance.v1.ListInstanceTypesResponse.InstanceType.server_attachable_quantity:type_name -> api.instance.v1.ListInstanceTypesResponse.InstanceType.ServerAttachableQuantity
	4,  // 36: api.instance.v1.Instance.ListRegions:input_type -> api.instance.v1.ListRegionsRequest
	6,  // 37: api.instance.v1.Instance.ListImages:input_type -> api.instance.v1.ListImagesRequest
	8,  // 38: api.instance.v1.Instance.ListInstanceTypes:input_type -> api.instance.v1.ListInstanceTypesRequest
	0,  // 39: api.instance.v1.Instance.CreateInstances:input_type -> api.instance.v1.CreateInstancesRequest
	2,  // 40: api.instance.v1.Instance.ListInstances:input_type -> api.instance.v1.ListInstancesRequest
	43, // 41: api.instance.v1.Instance.ShowJob:input_type -> api.instance.v1.ShowJobRequest
	10, // 42: api.instance.v1.Instance.StartInstances:input_type -> api.instance.v1.StartInstancesRequest
	12, // 43: api.instance.v1.Instance.StopInstances:input_type -> api.instance.v1.StopInstancesRequest
	14, // 44: api.instance.v1.Instance.RebootInstances:input_type -> api.instance.v1.RebootInstancesRequest
	17, // 45: api.instance.v1.Instance.DeleteInstances:input_type -> api.instance.v1.DeleteInstancesRequest
	21, // 46: api.instance.v1.Instance.CreateVpc:input_type -> api.instance.v1.CreateVpcRequest
	27, // 47: api.instance.v1.Instance.DeleteVpc:input_type -> api.instance.v1.DeleteVpcRequest
	23, // 48: api.instance.v1.Instance.ListVpcs:input_type -> api.instance.v1.ListVpcsRequest
	29, // 49: api.instance.v1.Instance.ShowVpc:input_type -> api.instance.v1.ShowVpcRequest
	25, // 50: api.instance.v1.Instance.UpdateVpc:input_type -> api.instance.v1.UpdateVpcRequest
	33, // 51: api.instance.v1.Instance.CreateSubnet:input_type -> api.instance.v1.CreateSubnetRequest
	39, // 52: api.instance.v1.Instance.DeleteSubnet:input_type -> api.instance.v1.DeleteSubnetRequest
	23, // 53: api.instance.v1.Instance.ListSubnets:input_type -> api.instance.v1.ListVpcsRequest
	41, // 54: api.instance.v1.Instance.ShowSubnet:input_type -> api.instance.v1.ShowSubnetRequest
	37, // 55: api.instance.v1.Instance.UpdateSubnet:input_type -> api.instance.v1.UpdateSubnetRequest
	5,  // 56: api.instance.v1.Instance.ListRegions:output_type -> api.instance.v1.ListRegionsResponse
	7,  // 57: api.instance.v1.Instance.ListImages:output_type -> api.instance.v1.ListImagesResponse
	9,  // 58: api.instance.v1.Instance.ListInstanceTypes:output_type -> api.instance.v1.ListInstanceTypesResponse
	1,  // 59: api.instance.v1.Instance.CreateInstances:output_type -> api.instance.v1.CreateInstancesResponse
	3,  // 60: api.instance.v1.Instance.ListInstances:output_type -> api.instance.v1.ListInstancesResponse
	44, // 61: api.instance.v1.Instance.ShowJob:output_type -> api.instance.v1.ShowJobResponse
	11, // 62: api.instance.v1.Instance.StartInstances:output_type -> api.instance.v1.StartInstancesResponse
	13, // 63: api.instance.v1.Instance.StopInstances:output_type -> api.instance.v1.StopInstancesResponse
	15, // 64: api.instance.v1.Instance.RebootInstances:output_type -> api.instance.v1.RebootInstancesResponse
	18, // 65: api.instance.v1.Instance.DeleteInstances:output_type -> api.instance.v1.DeleteInstancesResponse
	22, // 66: api.instance.v1.Instance.CreateVpc:output_type -> api.instance.v1.CreateVpcResponse
	28, // 67: api.instance.v1.Instance.DeleteVpc:output_type -> api.instance.v1.DeleteVpcResponse
	24, // 68: api.instance.v1.Instance.ListVpcs:output_type -> api.instance.v1.ListVpcsResponse
	30, // 69: api.instance.v1.Instance.ShowVpc:output_type -> api.instance.v1.ShowVpcResponse
	26, // 70: api.instance.v1.Instance.UpdateVpc:output_type -> api.instance.v1.UpdateVpcResponse
	34, // 71: api.instance.v1.Instance.CreateSubnet:output_type -> api.instance.v1.CreateSubnetResponse
	40, // 72: api.instance.v1.Instance.DeleteSubnet:output_type -> api.instance.v1.DeleteSubnetResponse
	36, // 73: api.instance.v1.Instance.ListSubnets:output_type -> api.instance.v1.ListSubnetsResponse
	42, // 74: api.instance.v1.Instance.ShowSubnet:output_type -> api.instance.v1.ShowSubnetResponse
	38, // 75: api.instance.v1.Instance.UpdateSubnet:output_type -> api.instance.v1.UpdateSubnetResponse
	56, // [56:76] is the sub-list for method output_type
	36, // [36:56] is the sub-list for method input_type
	36, // [36:36] is the sub-list for extension type_name
	36, // [36:36] is the sub-list for extension extendee
	0,  // [0:36] is the sub-list for field type_name
}

func init() { file_api_instance_v1_instance_proto_init() }
func file_api_instance_v1_instance_proto_init() {
	if File_api_instance_v1_instance_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_api_instance_v1_instance_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateInstancesRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[1].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateInstancesResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[2].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstancesRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[3].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstancesResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[4].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListRegionsRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[5].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListRegionsResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[6].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListImagesRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[7].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListImagesResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[8].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstanceTypesRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[9].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstanceTypesResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[10].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*StartInstancesRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[11].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*StartInstancesResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[12].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*StopInstancesRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[13].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*StopInstancesResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[14].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*RebootInstancesRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[15].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*RebootInstancesResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[16].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ServerId); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[17].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*DeleteInstancesRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[18].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*DeleteInstancesResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[19].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Vpc); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[20].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*VpcRoute); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[21].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateVpcRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[22].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateVpcResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[23].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListVpcsRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[24].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListVpcsResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[25].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UpdateVpcRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[26].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UpdateVpcResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[27].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*DeleteVpcRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[28].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*DeleteVpcResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[29].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ShowVpcRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[30].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ShowVpcResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[31].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*Subnet); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[32].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ExtraDhcpOpts); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[33].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateSubnetRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[34].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateSubnetResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[35].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListSubnetsRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[36].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListSubnetsResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[37].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UpdateSubnetRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[38].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*UpdateSubnetResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[39].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*DeleteSubnetRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[40].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*DeleteSubnetResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[41].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ShowSubnetRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[42].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ShowSubnetResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[43].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ShowJobRequest); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[44].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ShowJobResponse); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[45].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateInstancesRequest_Nic); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[46].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*CreateInstancesRequest_RootVolume); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[47].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstancesResponse_Instance); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[48].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstancesResponse_Instance_ServerAddress); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[49].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstancesResponse_Instance_ServerAddresses); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[51].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstancesResponse_Instance_InstanceType); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[52].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstancesResponse_Instance_SecurityGroup); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[53].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstancesResponse_Instance_ServerFault); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[55].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstancesResponse_Instance_ServerExtendVolumeAttachment); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[56].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstancesResponse_Instance_ServerSchedulerHints); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[57].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstancesResponse_Instance_ServerSystemTag); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[58].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstancesResponse_Instance_CpuOptions); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[59].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstancesResponse_Instance_Hypervisor); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[60].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListRegionsResponse_Region); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[61].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListImagesResponse_Image); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[62].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstanceTypesResponse_InstanceType); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[63].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstanceTypesResponse_InstanceType_OsExtraSpec); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
		file_api_instance_v1_instance_proto_msgTypes[64].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*ListInstanceTypesResponse_InstanceType_ServerAttachableQuantity); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_api_instance_v1_instance_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   65,
			NumExtensions: 0,
			NumServices:   1,
		},
		GoTypes:           file_api_instance_v1_instance_proto_goTypes,
		DependencyIndexes: file_api_instance_v1_instance_proto_depIdxs,
		MessageInfos:      file_api_instance_v1_instance_proto_msgTypes,
	}.Build()
	File_api_instance_v1_instance_proto = out.File
	file_api_instance_v1_instance_proto_rawDesc = nil
	file_api_instance_v1_instance_proto_goTypes = nil
	file_api_instance_v1_instance_proto_depIdxs = nil
}
